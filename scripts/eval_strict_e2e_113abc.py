...
Evaluates 112C Champion (1121.24 LB) vs 113A vs 113B vs 113C.
"""
import json
import os
import shutil
...
def evaluate_production_package(pkg_path: Path, val_df: pd.DataFrame, n_rows: int = 15000):
    ...
    sandbox = Path(tempfile.mkdtemp(prefix="strict_113_"))
    try:
        pkg_dir = sandbox / "pkg"
...
def main():
    print("=" * 80)
    print("  STRICT REAL END-TO-END EVALUATION: 112C (1121.24 LB) vs 113A vs 113B vs 113C  ")
    print("=" * 80)
    ...
    candidates = [
        ("112C Champion Base", ROOT / "model/REF4-DIRECT-BRIER-SIMPLEX-112C/production_package"),
        ("113A Disjoint EB Expert", ROOT / "model/REF4-DISJOINT-EB-113A/production_package"),
        ("113B Tunneling Specialist", ROOT / "model/REF4-TUNNELING-GBDT-113B/production_package"),
        ("113C Beta Calibration", ROOT / "model/REF4-BETA-SIMPLEX-113C/production_package"),
    ]
    ...
    print("\n" + "=" * 80)
    print("  FINAL 113 SCOREBOARD (TRUE END-TO-END HOLD-OUT EVALUATION)  ")
    print("=" * 80)
    print(f"{'Candidate':<26} | {'Brier Score':<12} | {'BSS (%)':<10} | {'Delta vs 112C':<14} | {'Speed'}")
    print("-" * 80)
