...
Evaluates 113A Champion (1121.90 LB) vs 114A vs 114B vs 114C.
"""
import json
import os
import shutil
...
def evaluate_production_package(pkg_path: Path, val_df: pd.DataFrame, n_rows: int = 15000):
    ...
    sandbox = Path(tempfile.mkdtemp(prefix="strict_114_"))
    try:
        pkg_dir = sandbox / "pkg"
...
def main():
    print("=" * 80)
    print("  STRICT REAL END-TO-END EVALUATION: 113A (1121.90 LB) vs 114A vs 114B vs 114C  ")
    print("=" * 80)
    ...
    candidates = [
        ...
        ("114A 10th Repo Full Core", ROOT / "model/REF4-EB-V66-NESTED-114A/production_package"),
        ("114B Dynamic Sigmoidal", ROOT / "model/REF4-NESTED-DYNAMIC-114B/production_package"),
        ("114C Tunneling Convex Logit", ROOT / "model/REF4-EXPERT-CONVEX-114C/production_package"),
    ]
    ...
    print("\n" + "=" * 80)
    print("  FINAL 114 SCOREBOARD (TRUE END-TO-END HOLD-OUT EVALUATION)  ")
    print("=" * 80)
