#!/usr/bin/env python3
"""
REF4-V66-TUNNELING-116C Builder.
Candidate 116C: 116B + 1.47M Row Pitch-Tunneling Specialist GBDT Direct Ensemble.
"""
import json
import os
import shutil
...
OUT_DIR = ROOT / "model/REF4-V66-TUNNELING-116C/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_116C.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 116C: REF4-V66-TUNNELING-116C  ")
    print("=" * 80)
    ...
    manifest["pipeline"] = "REF4-V66-TUNNELING-116C"
    manifest["description"] = "116B + 1.47M Row Pitch-Tunneling Specialist GBDT Direct Additive Ensemble"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["w_batter_exposure"] = 0.50
    manifest["w_batter_res"] = 0.25
    manifest["w_tunneling"] = 0.020
    ...
    script_code = '''# Production inference entry point for REF4-V66-TUNNELING-116C
import json
import os
import sys
import time
...
def main():
    ...
    # 1.47M Pitch Tunneling Dual Expert
    w_tun = float(meta.get("w_tunneling", 0.020))
    ...
    # Continuous Sigmoidal Leverage Scaling
    ...
    # =========================================================================
    # 116C: 10th Repo DIRECT ADDITIVE v66 Nested EB + v59 Batter Exposure
    # =========================================================================
    ...
    p = p + v66_delta + 0.50 * v59_delta - 0.25 * v61_delta
