#!/usr/bin/env python3
"""Build Production Package 099A: Champion 097A Backbone (1104.9303 LB) High-Capacity Convergence Retraining (Depth=7, Leaves=45, Iters=350, 20 Pure L2 Models)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-099A'
...
SOURCE_097_MODEL = ROOT / 'model/REF4-DEEP-HIERARCHICAL-097A/production_package/model'
SOURCE_097_SRC = ROOT / 'model/REF4-DEEP-HIERARCHICAL-097A/production_package/src'
SOURCE_097_REQS = ROOT / 'model/REF4-DEEP-HIERARCHICAL-097A/production_package/requirements.txt'
...
SCRIPT_CONTENT_099 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-099A."""
...
def main():
    ...
    # Pre-build v54 features for 099A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
    ...
    # 099A HIGH-CAPACITY BACKBONE: 20 Pure L2 Models Retrained on Hierarchical Reliability Residuals (Depth=7, Leaves=45, Iters=350)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Retraining 20 High-Capacity Pure L2 Models on 097A Hierarchical Reliability Baseline (Depth=7, Leaves=45, Iters=350) ===")
    ...
    prev1_tr = np.where(np.isnan(prev1_tr), p_rate_tr, prev1_tr)
    
    overall_prior = 0.523766
    k_prior = 25.0
    rel_weight = n_p_tr / (n_p_tr + k_prior)
    p_shrunk_tr = overall_prior + rel_weight * (p_rate_tr - overall_prior)
    p_hb_hier_tr = np.clip(0.70 * p_shrunk_tr + 0.30 * prev1_tr, 0.05, 0.95)
    
    y_tr = train_reg.control_success.to_numpy(float)
    resid_hb_tr = (y_tr - p_hb_hier_tr).astype(np.float32)
    
    v54_seeds = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    tr_res_preds = []
    
    for s in v54_seeds:
        # High-Capacity CatBoost L2 (Depth=7, Iters=350, LR=0.04)
        cb_m = CatBoostRegressor(iterations=350, depth=7, learning_rate=0.04, l2_leaf_reg=5, random_seed=s, verbose=False)
        ...
        # High-Capacity LightGBM L2 (Depth=7, Leaves=45, Rounds=280, LR=0.04)
        ...
        lgb_m = lgb.train(params, lgb_tr, num_boost_round=280)
        lgb_path = MODEL_DIR / f"deep_lgb_l2_seed{s}.txt"
        lgb_m.save_model(str(lgb_path))
        tr_res_preds.append(lgb_m.predict(v54_train_feat))
        
        print(f"  • Seed {s}: Retrained High-Capacity Deep L2 CatBoost (Depth 7, Iters 350) & LightGBM (Leaves 45, Rounds 280)")
    ...
    print("\n=== Step 2: Copying Champion 097A Assets & Manifest Setup ===")
    for f in SOURCE_097_MODEL.iterdir():
    ...
    for f in SOURCE_097_SRC.iterdir():
        if f.is_file():
    ...
    shutil.copy2(SOURCE_097_REQS, PROD_DIR / 'requirements.txt')
    
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_097_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-099A'
    ...
    manifest['notes'] = "099A High-Capacity Convergence Retraining (Depth=7, Leaves=45, Iters=350, 20 Models) on Champion 097A Baseline"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print(f"Updated manifest.json for 099A (deep_mean_offset = {deep_mean_offset:.6f})")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_099.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_099, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_099.zip'
    if zip_path.exists():
        zip_path.unlink()
