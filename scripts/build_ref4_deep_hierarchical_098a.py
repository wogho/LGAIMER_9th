#!/usr/bin/env python3
"""Build Production Package 098A: New Champion 097A Backbone (1104.9303 LB) + Futures Psych-Latent Scale Boost (0.15 -> 0.22)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-098A'
...
SCRIPT_CONTENT_098 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-098A."""
...
def main():
    ...
    if futures.any():
        ...
        f_psych_scale = float(meta.get("f_psych_scale", 1.4667))  # 1.4667 * 0.15 ~= 0.22
        p = np.where(futures, p + f_psych_scale * correction, p)

    # Pre-build v54 features for 098A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
    ...
    # 097A Champion: 20 Pure L2 Models on Hierarchical Reliability Residuals
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 097A Models & Building 098A Package ===")
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 098A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_097_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-098A'
    ...
    manifest['f_psych_scale'] = 1.4667  # Boost Futures psych scale from 0.15 to ~0.22
    manifest['v54_seeds'] = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    manifest['notes'] = "098A Champion 097A Backbone (1104.9303 LB) + Boosted Futures Psych Scale (0.22) strictly for Gate 7 100% row-independence compliance"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 098A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_098.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_098, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_098.zip'
    if zip_path.exists():
        zip_path.unlink()
