#!/usr/bin/env python3
"""
REF4-BT-SEASON-DECOMP-115A Builder.
Candidate 115A: 114B (113A + 10th Repo) + 9th Repo Bradley-Terry Latent Strength & Season Decomposition.
"""
import json
import os
import shutil
...
MODEL_114B = ROOT / "model/REF4-NESTED-DYNAMIC-114B/production_package/model"
SRC_114B = ROOT / "model/REF4-NESTED-DYNAMIC-114B/production_package/src"
FROZEN_9TH = ROOT / "model/frozen_9th_tables"
OUT_DIR = ROOT / "model/REF4-BT-SEASON-DECOMP-115A/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_115A.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 115A: REF4-BT-SEASON-DECOMP-115A  ")
    print("=" * 80)
    ...
    if SRC_114B.exists():
        for py_file in SRC_114B.glob("*.py"):
            shutil.copy2(py_file, out_src / py_file.name)
            
    # 2. Copy all 114B models
    for asset in MODEL_114B.glob("*"):
        if asset.is_file() and not (out_model / asset.name).exists():
            shutil.copy2(asset, out_model / asset.name)
            
    # 3. Copy 9th repo frozen tables
    for tbl in FROZEN_9TH.glob("*.json"):
        shutil.copy2(tbl, out_model / tbl.name)
        
    # 4. Write manifest.json
    manifest = json.loads((MODEL_114B / "manifest.json").read_text(encoding="utf-8"))
    manifest["pipeline"] = "REF4-BT-SEASON-DECOMP-115A"
    manifest["description"] = "114B + 9th Repo Bradley-Terry Latent Strengths & Season Decomposition"
    ...
    script_code = '''# Production inference entry point for REF4-BT-SEASON-DECOMP-115A
import json
import os
import sys
import time
...
def main():
    ...
    # =========================================================================
    # 10th Repo Full Core Integration (v66 Nested EB + v59/v61 Exposure)
    # =========================================================================
    v66_delta = apply_10th_repo_v66_deviations(test, MODEL / "v66_nested_deviations.json")
    
    v59_delta = np.zeros(len(test), dtype=float)
    if (MODEL / "v59_batter_exposure.json").exists():
        v59_conf = json.loads((MODEL / "v59_batter_exposure.json").read_text(encoding="utf-8"))
        lut_v59 = dict(zip(v59_conf["keys"], v59_conf["deltas"]))
        v59_delta = test["batter_id"].astype(str).map(lut_v59).fillna(0.0).to_numpy(float)
        
    v61_delta = np.zeros(len(test), dtype=float)
    if (MODEL / "v61_batter_residual.json").exists():
        v61_conf = json.loads((MODEL / "v61_batter_residual.json").read_text(encoding="utf-8"))
        lut_v61 = dict(zip(v61_conf["keys"], v61_conf["deltas"]))
        v61_delta = test["batter_id"].astype(str).map(lut_v61).fillna(0.0).to_numpy(float)
        
    # =========================================================================
    # 115A: 9th Repo Bradley-Terry Latent Strengths & Season Decomposition
    # =========================================================================
    p_bt = apply_9th_repo_bradley_terry(test, MODEL / "bradley_terry_eb.json")
    p_bt_clip = np.clip(p_bt, 1e-5, 1.0 - 1e-5)
    logit_p_bt = np.log(p_bt_clip / (1.0 - p_bt_clip))
    logit_bt_delta = logit_p_bt - np.log(0.523766 / (1.0 - 0.523766))
    
    season_decomp_gap = apply_9th_repo_season_decomp_gap(test, MODEL / "season_decomposition.json")

    # Logit-Space Linear Fusion
    ...
    logit_p_new = logit_p + w_v66 * v66_delta + w_v59 * v59_delta - w_v61 * v61_delta + w_bt * logit_bt_delta + w_season * season_decomp_gap
