#!/usr/bin/env python3
"""Build Production Package 093A: 091A Champion Backbone (1104.6158 LB) + Strict 12 Ball-Strike Count Cell Residual Engine (Count Level Matrix Calibration)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-093A'
...
SCRIPT_CONTENT_093 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-093A."""
...
def main():
    ...
    count_devs = np.zeros(len(test), dtype=np.float32)
    for idx in range(len(test)):
        b_val = min(max(b_col[idx], 0), 3)
        s_val = min(max(s_col[idx], 0), 2)
        count_devs[idx] = count_cell_map.get((b_val, s_val), 0.0)
        
    p = np.where(regular, p + w_count * count_devs, p)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 091A Models & Building 093A Package ===")
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 093A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_091_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-093A'
    ...
    manifest['w_count_cell'] = 0.50
    manifest['v54_seeds'] = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    manifest['notes'] = "091A Champion Backbone (1104.6158 LB) + 12 Ball-Strike Count Cell Matrix Residual Engine (w_count=0.50)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 093A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_093.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_093, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_093.zip'
    if zip_path.exists():
        zip_path.unlink()
