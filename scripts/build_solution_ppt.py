...
def build_deck():
    ...
    add_text(slide, "검증 상태  |  Strict Temporal Forward Split (2019-2023 -> 2024 Holdout)", 0.88, 6.48, 6.62, 0.27, size=11, color=TEAL, bold=True)
    ...
    add_rect(slide, 0.62, 4.63, 12.10, 1.70, "11222D", radius=True, line=TEAL, width=1.6)
    add_text(slide, "최종 공식 검증 및 무결성 감사 상태", 0.96, 4.90, 7.15, 0.36, size=16, color=INK, bold=True)
    add_text(slide, "AUDIT PASS", 8.80, 4.80, 3.20, 0.66, size=30, color=TEAL, bold=True, align=PP_ALIGN.CENTER)
    add_text(slide, "행 독립성 오차 0.000e+00  ·  Fail-Fast 계약 준수  ·  3-Tier Multi-Family GBDT Super Ensemble (113A)", 0.96, 5.55, 11.20, 0.28, size=11, color=MUTED)
