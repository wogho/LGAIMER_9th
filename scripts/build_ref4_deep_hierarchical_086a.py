#!/usr/bin/env python3
"""Build Production Package 086A with Active Season Reconstruction 75-Feature 20 Pure L2 Multi-Seed Deep Residual Engine."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-086A'
...
from v53_asof_reconstruct_75_features import build_v53_asof_reconstruct_75_features, build_priors_from_train
...
SCRIPT_CONTENT_086 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-086A."""
...
from src.v53_asof_reconstruct_75_features import build_v53_asof_reconstruct_75_features
...
def main():
    ...
    priors = json.loads((MODEL / "career_priors.json").read_text(encoding="utf-8")) if (MODEL / "career_priors.json").exists() else {}
    ...
    # NEW 086A: 20 Pure L2 Multi-Seed Deep Hierarchical Active Season Reconstruction Engine (75 features)
    w_deep = float(meta.get("w_deep_hierarchical", 0.08))
    v53_seeds = meta.get("v53_seeds", [42, 1, 2, 3, 4, 10, 20, 30, 40, 50])
    if regular.any() and w_deep > 0.0:
        v53_feat, p_hb = build_v53_asof_reconstruct_75_features(
        ...
        for s in v53_seeds:
            ...
            res_preds.append(cb_m.predict(v53_feat))
            
            lgb_m = lgb.Booster(model_file=str(MODEL / f"deep_lgb_l2_seed{s}.txt"))
            res_preds.append(lgb_m.predict(v53_feat))
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Extracting Career Priors & Training 20 Pure L2 75-Feature Model ===")
    raw = pd.read_csv(ROOT / 'data/train.csv', low_memory=False)
    priors = build_priors_from_train(raw)
    
    # Save career priors JSON
    (MODEL_DIR / 'career_priors.json').write_text(json.dumps(priors, indent=2), encoding='utf-8')
    
    train_reg = raw.loc[raw.game_type != "F"].copy().reset_index(drop=True)
    profile_path = ROOT / 'model/team_asof_profile.json'
    
    v53_train_feat, p_hb_tr = build_v53_asof_reconstruct_75_features(train_reg, profile_path=profile_path, priors=priors)
    
    y_tr = train_reg.control_success.to_numpy(float)
    resid_hb_tr = (y_tr - p_hb_tr).astype(np.float32)
    
    v53_seeds = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    tr_res_preds = []
    ...
    for s in v53_seeds:
        ...
        cb_m.fit(v53_train_feat, resid_hb_tr)
        cb_path = MODEL_DIR / f"deep_cb_l2_seed{s}.cbm"
        cb_m.save_model(str(cb_path))
        tr_res_preds.append(cb_m.predict(v53_train_feat))
        
        # LightGBM L2
        lgb_tr = lgb.Dataset(v53_train_feat, label=resid_hb_tr)
        params = {
        ...
        tr_res_preds.append(lgb_m.predict(v53_train_feat))
        
        print(f"  • Seed {s}: Deep Pure L2 CatBoost & LightGBM trained on 75 Active-Season Reconstructed features.")
    ...
    shutil.copy2(ROOT / 'src/v53_asof_reconstruct_75_features.py', SRC_DIR / 'v53_asof_reconstruct_75_features.py')
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-086A'
    ...
    manifest['v53_seeds'] = v53_seeds
    manifest['notes'] = "082A Champion Backbone (1104.12 LB) + Active Season Reconstruction 75-Feature Pure L2 Residual Engine (w=0.08) strictly on R"
    ...
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_086.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_086, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_086.zip'
    if zip_path.exists():
        zip_path.unlink()
