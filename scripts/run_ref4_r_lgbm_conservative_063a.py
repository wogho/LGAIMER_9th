#!/usr/bin/env python3
"""Run REF4-TRAINONLY-R-LGBM-CONSERVATIVE-063A forward OOF training and evaluation."""
...
OUT = ROOT / 'model/REF4-TRAINONLY-R-LGBM-CONSERVATIVE-063A'
...
def main():
    ...
    for valid_year in (2023, 2024):
        ...
        p = c['r_expert_params']
        print(f"Fitting LightGBM Regressor on {mask_r.sum()} R-rows...", flush=True)
        ...
        model_path = OUT / f"r_expert_lgbm_{valid_year}.txt"
        model.booster_.save_model(str(model_path))
        
        res_pred = model.predict(X_valid_v3)
        r_pred_valid = np.clip(base_valid + res_pred + 0.0052, 1e-6, 1.0 - 1e-6)
        ...
        print(f"LightGBM correlation with 051 on R-rows: {corr_with_051:.4f}", flush=True)
        ...
        fold_df = pd.DataFrame({
            ...
            'r_expert_lgbm_prediction': r_pred_valid,
            'candidate_prediction': primary_cand
        })
        oof_rows.append(fold_df)
        
        del X_train_v3, X_valid_v3, train_hist, valid_hist, ps, bs, ms, model
        gc.collect()
    ...
    result = {
        ...
        'candidate_name': f'R_lgbm_expert_blend_w{primary_w:.2f}_plus_fixed_051_and_F_psych',
    ...
    paths = [
        ...
        OUT / 'r_expert_lgbm_2023.txt', OUT / 'r_expert_lgbm_2024.txt',
        ...
        ROOT / '01_제약과금지사항.md',
        ROOT / 'scripts/preflight_ref4_r_lgbm_conservative_063a.py',
        ROOT / 'scripts/run_ref4_r_lgbm_conservative_063a.py',
        ROOT / 'scripts/verify_ref4_r_lgbm_conservative_063a.py'
    ...
    print(f"\n063A Execution finished in {time.time()-t0:.2f}s. Result: {result['candidate_status']}, Promotion: {promotion}")
