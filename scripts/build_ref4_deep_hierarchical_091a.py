#!/usr/bin/env python3
"""Build Production Package 091A: Champion 087A Backbone (1104.61 LB) + Dual Tier-Aware Level Grouping Shrinkage (Symmetric Pocket Post-Processing for Overconfident High/Low Cold-Start Pitchers)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-091A'
...
SCRIPT_CONTENT_091 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-091A."""
...
def main():
    ...
    # 091A NEW: Dual Tier-Aware Level Grouping Pocket Post-Processor (Symmetric Cold-Start Shrinkage)
    ...
    # High-pocket: Cold-start pitchers with small n (< 15) but inflated high past rate (> 0.65) -> Downshift
    ...
    # Low-pocket: Cold-start pitchers with small n (< 15) but artificially low past rate (< 0.40) -> Upshift to mean
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 087A Models & Building 091A Package ===")
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 091A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_087_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-091A'
    ...
    manifest['notes'] = "087A Champion Backbone (1104.61 LB) + Level Grouping Dual Pocket Shrinkage (High Downshift -0.012, Low Upshift +0.008)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 091A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_091.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_091, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_091.zip'
    if zip_path.exists():
        zip_path.unlink()
