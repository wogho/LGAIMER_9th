#!/usr/bin/env python3
"""
CAT-RONLY-DEPTH7-001 (Audited & Automated)
Evaluation of Depth-7 R-only CatBoost diversity and 3-way multi-model blends.
...
OUTPUT_DIR = ROOT / "model" / "CAT-RONLY-DEPTH7-001"
...
def run_catboost_d7_eval() -> tuple[dict[str, Any], str]:
    ...
    candidate_configs = {
        ...
        "d7_cand1_timeweighted_top (LGBM_R 0.50 : CAT_D7 0.50)": (0.50, 0.50, 0.00),
        "d7_cand2 (LGBM_R 0.60 : CAT_D7 0.40)": (0.60, 0.40, 0.00),
        "d7_cand3_3way_balanced (LGBM_R 0.50 : CAT_D7 0.25 : CAT_FULL 0.25)": (0.50, 0.25, 0.25),
        "d7_cand4_3way_catheavy (LGBM_R 0.40 : CAT_D7 0.30 : CAT_FULL 0.30)": (0.40, 0.30, 0.30),
        "d7_cand5_2024_top (LGBM_R 0.35 : CAT_D7 0.35 : CAT_FULL 0.30)": (0.35, 0.35, 0.30),
    }
    
    candidates_results: dict[str, Any] = {}
    
    for cand_name, (w_lgbm_r, w_cat_d7, w_cat_full) in candidate_configs.items():
        raw_preds = {
            s: build_candidate_preds(frames[s], w_lgbm_r, w_cat_d7, w_cat_full)
            for s in [2022, 2023, 2024]
        }
        ...
        candidates_results[cand_name] = {
            "weights": {"lgbm_r": w_lgbm_r, "cat_d7": w_cat_d7, "cat_full": w_cat_full},
    ...
    lines = [
        "# CAT-RONLY-DEPTH7-001: Depth-7 R-only CatBoost 다양성 벤치마크 (정정 감사 완료)",
        "",
    ...
    c1 = candidates_results["d7_cand1_timeweighted_top (LGBM_R 0.50 : CAT_D7 0.50)"]
    c5 = candidates_results["d7_cand5_2024_top (LGBM_R 0.35 : CAT_D7 0.35 : CAT_FULL 0.30)"]
    
    d24_c5 = c5["seasons"][2024]["delta_vs_sub002"]
    proxy24_c5 = c5["seasons"][2024]["local_cv_proxy_score"]
    w_c5 = c5["temporal_weighted_delta_vs_sub002"]
    
    w_c1 = c1["temporal_weighted_delta_vs_sub002"]
    proxy24_c1 = c1["seasons"][2024]["local_cv_proxy_score"]
    d24_c1 = c1["seasons"][2024]["delta_vs_sub002"]
    
    lines.extend([
        "",
        "## 2. Depth-7 구조 다양성 효과 분석 (JSON 실측치 기반 동적 생성)",
        "",
        f"1. **2024 성능 우선 후보 (`d7_cand5_2024_top`)**:",
        f"  - 2024 Brier Δ vs SUB-002: `{d24_c5:+.7f}`",
        f"  - 2024 Local CV Proxy Score: `{proxy24_c5:.2f}점`",
        f"  - 시간가중 Δ vs SUB-002: `{w_c5:+.7f}`",
        f"2. **시간가중 개선 우선 후보 (`d7_cand1_timeweighted_top`)**:",
        f"  - 시간가중 Δ vs SUB-002: `{w_c1:+.7f}` (전 시즌 고른 개선)",
        f"  - 2024 Brier Δ vs SUB-002: `{d24_c1:+.7f}` (local proxy `{proxy24_c1:.2f}점`)",
        "- **게이트 판정**: 어떤 후보도 2024 순수 개선폭이 개정 게이트(`-0.00010`)에 도달하지 못하였으므로, **SUB-003 제출 승인을 보류하고 연구 후보 상태(`research_candidate_not_submission_approved`)로 보존**함.",
    ])
    
    report_md = "\n".join(lines)
    
    results = {
        "experiment_id": "CAT-RONLY-DEPTH7-001",
        "runtime_environment": {
...
def main() -> None:
    results, report_md = run_catboost_d7_eval()
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    results_path = OUTPUT_DIR / "cat_d7_results.json"
    report_path = OUTPUT_DIR / "cat_d7_report.md"
