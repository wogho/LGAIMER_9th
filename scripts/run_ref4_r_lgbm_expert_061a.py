#!/usr/bin/env python3
"""Run REF4-TRAINONLY-R-LGBM-EXPERT-061A forward OOF training and evaluation."""
...
def main():
    ...
    for valid_year in (2023, 2024):
        ...
        print(f"Fitting R-only LightGBM Regressor on {mask_r.sum()} R-rows...", flush=True)
        p = c['r_expert_params']
        ...
        print(f"Prediction correlation with 051 on R-rows: {corr_with_051:.4f}", flush=True)
    ...
    paths = [
        ...
        ROOT / '01_제약과금지사항.md',
        ROOT / 'scripts/preflight_ref4_r_lgbm_expert_061a.py',
        ROOT / 'scripts/run_ref4_r_lgbm_expert_061a.py',
        ROOT / 'scripts/verify_ref4_r_lgbm_expert_061a.py'
    ...
    print(f"\n061A Execution finished in {time.time()-t0:.2f}s. Result: {result['candidate_status']}, Promotion: {promotion}")
