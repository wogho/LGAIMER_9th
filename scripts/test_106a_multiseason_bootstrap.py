#!/usr/bin/env python3
"""Multi-Season (2022, 2023, 2024) and Cluster Bootstrap Validation for 106A."""
...
def eval_season(season_num, raw, MODEL, meta, priors, ps, bs, ms, tm, seeds, regime, k_val, w_val):
    val_df = raw[raw["season"] == season_num].copy().reset_index(drop=True)
    ...
    # 106A Candidate with Reliability Shrinkage
    # rel_mod = (n_p_raw) / (n_p_raw + k_val)
    rel_mod = (n_p_raw) / (n_p_raw + k_val)
    p_106 = np.where(regular, p + w_val * (rel_mod * deep_corr) + 0.04 * call_corr, p)
    p_106 = np.where(high_pocket_mask, p_106 - 0.012, p_106)
    p_106 = np.where(low_pocket_mask, p_106 + 0.008, p_106)
    p_106 = np.clip(p_106, 1e-5, 1 - 1e-5)
    
    b_103 = brier_score(y_val, p_103)
    bss_103 = brier_skill_score(y_val, p_103)
    b_106 = brier_score(y_val, p_106)
    bss_106 = brier_skill_score(y_val, p_106)
    ...
    return {
        ...
        "brier_106": b_106,
        "bss_106": bss_106,
        "delta_brier": b_103 - b_106,
        "delta_bss": bss_106 - bss_103,
        "pitchers": val_df["pitcher_id"].to_numpy(),
        "y_true": y_val,
        "p_103": p_103,
        "p_106": p_106
    }
...
def main():
    print("=== Multi-Season Evaluation: 103A vs 106A (K=5.0, w=0.12) ===")
    ...
    for k_val, w_val in [(5.0, 0.10), (5.0, 0.12), (10.0, 0.12)]:
        print(f"\nEvaluating Configuration: K={k_val}, w={w_val}")
        for s in [2022, 2023, 2024]:
            res = eval_season(s, raw, MODEL, meta, priors, ps, bs, ms, tm, seeds, regime, k_val, w_val)
            print(f"  Season {s} ({res['n_rows']:,} rows) -> BSS 103A: {res['bss_103']:.4f} | BSS 106A: {res['bss_106']:.4f} (Δ {res['delta_bss']:+.4f} pt) | Δ Brier: {res['delta_brier']:+.8f}")
