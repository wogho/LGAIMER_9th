#!/usr/bin/env python3
"""Build Production Package 104A: New Champion 103A Backbone (1107.1970 LB) + Multi-Class Sub-Expert Engine Refined Location 5-Class Multi-Class Target Sub-Expert Integration."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-104A'
...
SCRIPT_CONTENT_104 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-104A."""
...
def main():
    ...
    # Pre-build v54 features for 104A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
    ...
    # 104A NEW: Multi-Class Sub-Expert Engine Refined Location 5-Class Multi-Class Sub-Expert (w=0.03)
    w_refined_loc = float(meta.get("w_refined_loc", 0.03))
    if regular.any() and w_refined_loc > 0.0 and (MODEL / "refined_loc_expert.cbm").exists():
        cb_loc = CatBoostClassifier()
        cb_loc.load_model(str(MODEL / "refined_loc_expert.cbm"))
        ...
        loc_probs = cb_loc.predict_proba(x3_cb)
        # class 0 = success, class 1 = reverse, class 2 = middle, class 3 = wild, class 4 = other
        p_loc_success = loc_probs[:, 0]
        loc_mean_offset = float(meta.get("loc_mean_offset", 0.514016))
        loc_corr = p_loc_success - loc_mean_offset
        p = np.where(regular, p + w_refined_loc * loc_corr, p)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Training Multi-Class Sub-Expert Engine Refined Location 5-Class Multi-Class CatBoost Sub-Expert ===")
    ...
    # Multi-Class Sub-Expert Engine Refined Location 5-Class Target 분해:
    # 0 = success, 1 = reverse, 2 = middle, 3 = wild, 4 = other_fail
    ...
    loc_target = np.zeros(len(train_reg), dtype=int)
    loc_target = np.where(succ == 1, 0, loc_target)
    loc_target = np.where((succ == 0) & (rev == 1), 1, loc_target)
    loc_target = np.where((succ == 0) & (mid == 1), 2, loc_target)
    loc_target = np.where((succ == 0) & (wld == 1), 3, loc_target)
    loc_target = np.where((succ == 0) & (rev == 0) & (mid == 0) & (wld == 0), 4, loc_target)
    ...
    cb_loc.fit(x3_cb_tr, loc_target)
    loc_expert_path = MODEL_DIR / "refined_loc_expert.cbm"
    cb_loc.save_model(str(loc_expert_path))
    
    tr_loc_probs = cb_loc.predict_proba(x3_cb_tr)[:, 0]
    loc_mean_offset = float(np.mean(tr_loc_probs))
    print(f"  • Trained Refined Location Multi-Class Expert (Mean success prob: {loc_mean_offset:.6f})")

    print("\n=== Step 2: Copying Champion 103A Models & Base Assets ===")
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-104A'
    ...
    manifest['w_refined_loc'] = 0.03
    manifest['loc_mean_offset'] = loc_mean_offset
    manifest['notes'] = "104A Champion 103A Backbone (1107.1970 LB) + Multi-Class Sub-Expert Engine Refined Location 5-Class Multi-Class Sub-Expert (w=0.03)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 104A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_104.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_104, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_104.zip'
    if zip_path.exists():
        zip_path.unlink()
