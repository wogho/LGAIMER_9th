#!/usr/bin/env python3
"""
REF4-BETA-SIMPLEX-113C Builder.
Candidate 113C: 112C Base + Exact Beta Calibration & Multi-Component Simplex Ensemble.
"""
import json
import os
import shutil
...
OUT_DIR = ROOT / "model/REF4-BETA-SIMPLEX-113C/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_113C.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 113C: REF4-BETA-SIMPLEX-113C  ")
    print("=" * 80)
    ...
    manifest["pipeline"] = "REF4-BETA-SIMPLEX-113C"
    manifest["description"] = "112C Base + Exact Beta Calibration & Multi-Component Simplex Ensemble"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["beta_a"] = 0.473477
    manifest["beta_c"] = -0.064668
    manifest["w_super_resid_base"] = 0.088
    manifest["w_refined_call_base"] = 0.042
    manifest["w_deep_hierarchical_base"] = 0.078
    ...
    script_code = '''# Production inference entry point for REF4-BETA-SIMPLEX-113C
import json
import os
import sys
import time
...
def apply_beta_calibration(p, a=0.473477, c=-0.064668):
    p_clip = np.clip(p, 1e-5, 1.0 - 1e-5)
    log_odds = np.log(p_clip / (1.0 - p_clip))
...
def main():
    ...
    if regular.any():
        ...
        # 113C Beta-Calibrated Core Anchor
        p_raw_anchor = 0.70 * p_rate_raw[regular] + 0.30 * prev1_raw
        p_hb_hier = apply_beta_calibration(p_raw_anchor, a=float(meta.get("beta_a", 0.473477)), c=float(meta.get("beta_c", -0.064668)))
    ...
    # SLSQP-Optimized Deep L2 Weight
