#!/usr/bin/env python3
"""
REF4-TUNNELING-GBDT-113B Builder.
Candidate 113B: 112C Base + Real Pitch-Tunneling & Count-Tension Dual Specialist Models.
"""
import json
import os
import shutil
...
MODEL_112C = ROOT / "model/REF4-DIRECT-BRIER-SIMPLEX-112C/production_package/model"
SRC_112C = ROOT / "model/REF4-DIRECT-BRIER-SIMPLEX-112C/production_package/src"
OUT_DIR = ROOT / "model/REF4-TUNNELING-GBDT-113B/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_113B.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 113B: REF4-TUNNELING-GBDT-113B  ")
    print("=" * 80)
    ...
    if SRC_112C.exists():
        for py_file in SRC_112C.glob("*.py"):
            shutil.copy2(py_file, out_src / py_file.name)
            
    # 2. Copy all 112C models
    for asset in MODEL_112C.glob("*"):
        if asset.is_file() and not (out_model / asset.name).exists():
            shutil.copy2(asset, out_model / asset.name)
            
    # 3. Write manifest.json
    manifest = json.loads((MODEL_112C / "manifest.json").read_text(encoding="utf-8"))
    manifest["pipeline"] = "REF4-TUNNELING-GBDT-113B"
    manifest["description"] = "112C Base + Real Pitch-Tunneling & Count-Tension Dual Specialist"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["w_tunneling_specialist"] = 0.038
    ...
    # 5. Write production script.py
    script_code = '''# Production inference entry point for REF4-TUNNELING-GBDT-113B
import json
import os
import sys
import time
...
def main():
    ...
    # 113B: Pitch-Tunneling & Count-Tension Dual Specialist
    if regular.any() and (MODEL / "tunneling_lgb.txt").exists():
        ...
        b = test.loc[regular, "balls_before"].fillna(0).astype(int).to_numpy() if "balls_before" in test.columns else np.zeros(int(np.sum(regular)), dtype=int)
        s = test.loc[regular, "strikes_before"].fillna(0).astype(int).to_numpy() if "strikes_before" in test.columns else np.zeros(int(np.sum(regular)), dtype=int)
        ...
        tun_X = pd.DataFrame({
            ...
            "pitcher_recent_drift": (prev1_reg - p_rate_reg) * (n_p / (n_p + 15.0)),
        ...
        w_tun = float(meta.get("w_tunneling_specialist", 0.038))
