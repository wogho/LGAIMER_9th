#!/usr/bin/env python3
"""
REF4-HEAVY-OOF-COMMAND-118 Production Packager & Builder.
Integrates 113A Champion (1121.9040) + 10th Repo v66 Direct Additive EB + 15 Heavy 5-Fold OOF GBDT Models (CatBoost, LightGBM, XGBoost).
"""
import json
import os
import shutil
...
OUT_DIR = ROOT / "model/REF4-HEAVY-OOF-COMMAND-118/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_118.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 118: REF4-HEAVY-OOF-COMMAND-118  ")
    print("=" * 80)
    ...
    # 3. Copy 10th repo frozen tables
    for tbl in FROZEN_10TH.glob("*.json"):
        if not (out_model / tbl.name).exists():
            shutil.copy2(tbl, out_model / tbl.name)
        
    # 4. Write manifest.json
    manifest = json.loads((MODEL_113A / "manifest.json").read_text(encoding="utf-8"))
    manifest["pipeline"] = "REF4-HEAVY-OOF-COMMAND-118"
    manifest["description"] = "113A Champion (1121.9040) + 10th Repo v66 Direct Additive EB + 15 Heavy 5-Fold OOF GBDTs"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["w_heavy_command"] = 0.045
    manifest["w_v66_direct"] = 1.0
    ...
    script_code = '''# Production inference entry point for REF4-HEAVY-OOF-COMMAND-118
import json
import os
import sys
import time
...
def predict_heavy_118_ensemble(df: pd.DataFrame, model_dir: Path, priors: dict) -> np.ndarray:
    feat, _ = build_v54_per_season_asof_75_features(
        df, priors=priors, prior=0.523766
    )
    b = df["balls_before"].fillna(0).to_numpy(float) if "balls_before" in df.columns else np.zeros(len(df))
    s = df["strikes_before"].fillna(0).to_numpy(float) if "strikes_before" in df.columns else np.zeros(len(df))
    li = df["li"].fillna(0.98).to_numpy(float) if "li" in df.columns else np.full(len(df), 0.98)
    ...
    feat["count_pressure"] = (b - s) * (b + s + 1.0) / 7.0
    feat["is_2s"] = (s == 2).astype(float)
    feat["is_3b"] = (b == 3).astype(float)
    feat["is_high_li"] = (li >= 1.5).astype(float)
    feat["late_close"] = ((inn >= 7) & (score_diff <= 2)).astype(float)
    
    preds = []
    for fold in range(1, 6):
        cb_path = model_dir / f"heavy_cb_fold{fold}.cbm"
        if cb_path.exists():
            cb = CatBoostRegressor()
            cb.load_model(str(cb_path))
            preds.append(safe_cb_predict(cb, feat))
            
        lgb_path = model_dir / f"heavy_lgb_fold{fold}.txt"
        if lgb_path.exists():
            lgbm = lgb.Booster(model_file=str(lgb_path))
            preds.append(safe_lgb_predict(lgbm, feat))
            
        xgb_path = model_dir / f"heavy_xgb_fold{fold}.json"
        if xgb_path.exists():
            xgb = XGBRegressor()
            xgb.load_model(str(xgb_path))
            preds.append(safe_xgb_predict(xgb, feat))
...
def main():
    ...
    # =========================================================================
    # 118: 10th Repo v66 Direct Additive EB + 15 Heavy 5-Fold OOF Command Ensemble
    # =========================================================================
    v66_delta = apply_10th_repo_v66_deviations(test, MODEL / "v66_nested_deviations.json")
    
    # 15 Heavy 5-Fold OOF Models Prediction
    p_heavy_cmd = predict_heavy_118_ensemble(test, MODEL, priors)
    w_heavy = float(meta.get("w_heavy_command", 0.045))
    
    p = p + v66_delta + w_heavy * (p_heavy_cmd - 0.523766)
