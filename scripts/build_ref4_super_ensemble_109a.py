#!/usr/bin/env python3
"""Build Production Package 109A: 108C Improvement - 15-Model Tri-Family (5 CatBoost + 5 LightGBM + 5 XGBoost) + Pitcher/Batter log(n) Curvature Shrinkage."""
...
EXP_DIR = ROOT / 'model/REF4-SUPER-ENSEMBLE-109A'
...
def extract_advanced_physics_109a(df: pd.DataFrame) -> pd.DataFrame:
    """3D 물리 궤적 + 볼카운트 압박 + log(n) 곡률 신뢰도 피처."""
    ...
    # 3번/Adaptive Hierarchical Gate log(n) Curvature
...
SCRIPT_CONTENT_109A = '''"""Offline inference entry point for REF4-SUPER-ENSEMBLE-109A."""
...
def main():
    ...
    # 109A: 15-Model Tri-Family (5 CatBoost + 5 LightGBM + 5 XGBoost) Residual Boosters (w=0.08)
    ...
    if regular.any() and w_super_resid > 0.0:
        ...
        phys_reg = extract_advanced_physics_109a(test[regular].reset_index(drop=True))
        ...
        for s in r_seeds:
            ...
            if xgb_path.exists():
                ...
                resid_reg_preds.append(xgb_m.predict(X_reg_full))
    ...
    if futures.any() and w_fut_resid > 0.0:
        ...
        phys_fut = extract_advanced_physics_109a(test[futures].reset_index(drop=True))
...
def build_package():
    ...
    # Copy requirements
    reqs_text = SOURCE_107_REQS.read_text()
    if 'xgboost' not in reqs_text:
        reqs_text += '\nxgboost\n'
    (PROD_DIR / 'requirements.txt').write_text(reqs_text.strip() + '\n')
    
    print("\n=== Step 2: Training 109A 15-Model Tri-Family Residual Boosters on Train (1,475,092 rows) ===")
    ...
    phys_reg = extract_advanced_physics_109a(raw_reg)
    X_physics_reg = pd.concat([X_61_reg, phys_reg], axis=1)
    
    r_seeds = [42, 1, 2, 3, 4]
    print(f"\nFitting 1군 15 Tri-Family Boosters (5 CatBoost + 5 LightGBM + 5 XGBoost) on {len(X_physics_reg):,} Regular rows...")
    for s in r_seeds:
        ...
        xgb = XGBRegressor(n_estimators=170, learning_rate=0.03, max_depth=4, subsample=0.8, colsample_bytree=0.8, random_state=s, n_jobs=-1, tree_method="hist")
        xgb.fit(X_physics_reg, y_resid_reg)
        xgb.save_model(str(MODEL_DIR / f"super_resid_xgb_seed{s}.json"))
        print(f"  • Regular Seed {s}: CatBoost, LightGBM, XGBoost fitted and saved")
    ...
    phys_fut = extract_advanced_physics_109a(raw_fut)
    X_physics_fut = pd.concat([X_61_fut, phys_fut], axis=1)
    
    f_seeds = [42, 1, 2, 3]
    print(f"\nFitting 2군 4 Futures Robust Boosters (4 CatBoost) on {len(X_physics_fut):,} Futures rows...")
    for s in f_seeds:
    ...
    print("\n=== Step 3: Updating Manifest for 109A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_107_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-SUPER-ENSEMBLE-109A'
    manifest['w_super_resid'] = 0.08
    manifest['w_fut_resid'] = 0.035
    manifest['r_seeds'] = r_seeds
    manifest['f_seeds'] = f_seeds
    manifest['notes'] = "109A Champion 108C + 15-Model Tri-Family (CatBoost+LightGBM+XGBoost) + log(n) Curvature Shrinkage"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    
    print("\n=== Step 4: Packaging submit_ref4_super_ensemble_109A.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_109A, encoding='utf-8')
