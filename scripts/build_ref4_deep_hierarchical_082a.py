#!/usr/bin/env python3
"""Build Production Package 082A with 10-Model (5 CatBoost + 5 LightGBM) Deep Hierarchical Base Residual Super Ensemble."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-082A'
...
from v5_deep_61_features import build_v5_deep_61_features
...
SCRIPT_CONTENT_082 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-082A."""
...
def main():
    ...
    # NEW 082A: 10-Model Deep Hierarchical Residual Model (984pt class) strictly for Regular season (game_type == 'R')
    w_deep = float(meta.get("w_deep_hierarchical", 0.08))
    v5_seeds = meta.get("v5_seeds", [42, 1, 2, 3, 4])
    if regular.any() and w_deep > 0.0:
        v5_feat, p_hb = build_v5_deep_61_features(test, profile_path=MODEL / "team_asof_profile.json", prior=float(meta.get("prior", 0.523766)))
        
        res_preds = []
        for s in v5_seeds:
            cb_m = CatBoostRegressor()
            cb_m.load_model(str(MODEL / f"deep_cb_seed{s}.cbm"))
            res_preds.append(cb_m.predict(v5_feat))
            
            lgb_m = lgb.Booster(model_file=str(MODEL / f"deep_lgb_seed{s}.txt"))
            res_preds.append(lgb_m.predict(v5_feat))
            
        p_deep_resid = np.mean(res_preds, axis=0)
        p_deep_reconstructed = np.clip(p_hb + p_deep_resid, 1e-5, 1 - 1e-5)
        deep_mean_offset = float(meta.get("deep_mean_offset", 0.5140248779020316))
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Training 10-Model Deep Hierarchical Residual Model ===")
    ...
    v5_train_feat, p_hb_tr = build_v5_deep_61_features(train_reg, profile_path=profile_path)
    ...
    for s in v5_seeds:
        ...
        cb_m.fit(v5_train_feat, resid_hb_tr)
        cb_path = MODEL_DIR / f"deep_cb_seed{s}.cbm"
        cb_m.save_model(str(cb_path))
        tr_res_preds.append(cb_m.predict(v5_train_feat))
        
        lgb_tr = lgb.Dataset(v5_train_feat, label=resid_hb_tr)
        params = {
        ...
        lgb_path = MODEL_DIR / f"deep_lgb_seed{s}.txt"
        lgb_m.save_model(str(lgb_path))
        tr_res_preds.append(lgb_m.predict(v5_train_feat))
        
        print(f"  • Seed {s}: Deep CatBoost & LightGBM trained on (y - p_hb) residual.")
    ...
    shutil.copy2(ROOT / 'src/v5_deep_61_features.py', SRC_DIR / 'v5_deep_61_features.py')
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-082A'
    ...
    manifest['notes'] = "079A Champion Backbone (1094.97 LB) + 10-Model Deep Hierarchical Residual Model (984pt class, w=0.08) strictly on R"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print(f"Updated manifest.json: w_deep_hierarchical = {manifest['w_deep_hierarchical']}")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_082.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_082, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_082.zip'
    if zip_path.exists():
        zip_path.unlink()
