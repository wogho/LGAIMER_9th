#!/usr/bin/env python3
"""Build Production Package 088A: Champion 087A Backbone (1104.61pt) + Upgraded 75-Feature LightGBM R-Expert + 20 Pure L2 Residual Ensemble."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-088A'
...
SOURCE_087_MODEL = ROOT / 'model/REF4-DEEP-HIERARCHICAL-087A/production_package/model'
SOURCE_087_SRC = ROOT / 'model/REF4-DEEP-HIERARCHICAL-087A/production_package/src'
SOURCE_087_REQS = ROOT / 'model/REF4-DEEP-HIERARCHICAL-087A/production_package/requirements.txt'

sys.path.insert(0, str(ROOT / 'src'))
from v54_per_season_asof_75_features import build_v54_per_season_asof_75_features, build_per_season_priors
...
SCRIPT_CONTENT_088 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-088A."""
...
def main():
    ...
    # Pre-build v54 features for R-Expert and Deep Hierarchical Engine
    v54_feat = None
    p_hb = None
    if regular.any():
    ...
    # Regular 1군 split profile + UPGRADED 75-Feature LightGBM R-Expert
    if regular.any():
        ...
        # Upgraded LightGBM R-Expert on 75 features (w=0.05)
        lgbm_model = lgb.Booster(model_file=str(MODEL / 'r_expert_lgbm_v54.txt'))
        res_lgbm = lgbm_model.predict(v54_feat)
        p_lgbm = np.clip(p_hb + res_lgbm, 1e-6, 1.0 - 1e-6)
        
        w_lgb = float(meta.get("r_expert_lgbm_weight", 0.05))
        p = np.where(regular, (1.0 - w_lgb) * p_split + w_lgb * p_lgbm, p)

    # 088A: 20 Pure L2 Multi-Seed Deep Hierarchical Per-Season Active State Engine (75 features)
    ...
    # Brier-Optimized Pocket Downshift Post-Processor
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 087A Models & Building 75-Feature LightGBM R-Expert ===")
    raw = pd.read_csv(ROOT / 'data/train.csv', low_memory=False)
    priors = build_per_season_priors(raw)
    
    # Copy all 087A model assets first
    for f in SOURCE_087_MODEL.iterdir():
        if f.is_file() and f.name != 'manifest.json':
            shutil.copy2(f, MODEL_DIR / f.name)
            
    for f in SOURCE_087_SRC.iterdir():
        if f.is_file():
    ...
    # Train NEW 75-Feature LightGBM R-Expert with n_jobs=4
    print("  • Training Upgraded 75-Feature LightGBM R-Expert (r_expert_lgbm_v54.txt)...")
    lgb_r_tr = lgb.Dataset(v54_train_feat, label=resid_hb_tr)
    params_r = {
    ...
    lgb_r_m = lgb.train(params_r, lgb_r_tr, num_boost_round=150)
    lgb_r_m.save_model(str(MODEL_DIR / 'r_expert_lgbm_v54.txt'))
    print(f"  • LightGBM R-Expert trained in {time.time() - t0:.2f}s!")

    print("\n=== Step 2: Updating Manifest and Packaging ===")
    ...
    shutil.copy2(SOURCE_087_REQS, PROD_DIR / 'requirements.txt')
    
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_087_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-088A'
    ...
    manifest['notes'] = "087A Champion Backbone (1104.61 LB) + Upgraded 75-Feature LightGBM R-Expert + 20 Pure L2 Residual Ensemble (w=0.08) strictly on R"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_088.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_088, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_088.zip'
    if zip_path.exists():
        zip_path.unlink()
