...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'model/REF4-TRAINONLY-R-STABLE-CONTEXT-058A'
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def main():
 ...
 paths=[ROOT/c[k] for k in ('official_train','reference_source','f_psych_predictions','f_psych_attestation','comparison_predictions','comparison_attestation','failed_predecessor_attestation','preserve_zip','leaderboard_evidence')]+[ROOT/'01_제약과금지사항.md',ROOT/'start03_reference.md',ROOT/'start04_uptostage.md',ROOT/'src/entity_context_split.py',ROOT/'src/stable_context_psych.py']+[ROOT/p for p in c['base_oof'].values()]
