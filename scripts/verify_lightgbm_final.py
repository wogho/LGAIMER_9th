#!/usr/bin/env python3
"""Verify an isolated full-data LightGBM candidate against its locked policy."""
...
from scripts.train_lightgbm_final import prediction_sha256, sha256_file  # noqa: E402
from src.features import build_features, load_data  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify final LightGBM candidate")
    parser.add_argument(
        "experiment_dir",
        type=Path,
        nargs="?",
...
def main() -> None:
    args = parse_args()
    metadata = json.loads((args.experiment_dir / "metadata.json").read_text(encoding="utf-8"))
    ...
    if metadata["sample_weight_used"] or metadata["test_predictions_computed"]:
    ...
    if sha256_file(model_path) != metadata["model_sha256"]:
        raise ValueError("native model 해시가 metadata와 다릅니다")
    ...
    if booster.num_trees() != metadata["n_estimators"]:
        raise ValueError("native model tree 계약이 다릅니다")
    if booster.feature_name() != feature_columns:
        raise ValueError("native model 피처 이름·순서가 다릅니다")
    prediction = booster.predict(features, num_iteration=metadata["n_estimators"])
    ...
    print(f"PASS native LightGBM load ({lgb.__version__}, {booster.num_trees()} trees)")
