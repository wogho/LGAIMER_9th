#!/usr/bin/env python3
"""
REF4-FAST-HIERARCHICAL-EB-111A Builder.
Features:
  1. Base Backbone: 109C Champion Backbone (Official LB: 1120.8914)
  2. Exact Beta-Binomial Hierarchical Empirical Bayes Table (model/eb_pitcher_hand_lookup.json)
  3. Dynamic Credibility Shrinkage: p_shrunk = gamma * p_asof + (1 - gamma) * p_eb
  4. Streamlined 8-Booster Residual Ensemble (3 LGB + 3 CB + 2 XGB) for <5.0 min inference
  5. 100% Column Permutation Invariance & Row Independence with safe wrappers
"""
import json
import os
import shutil
...
SRC_109 = ROOT / "model/REF4-SUPER-ENSEMBLE-109C/production_package/src"
OUT_DIR = ROOT / "model/REF4-FAST-HIERARCHICAL-EB-111A/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_111A.zip"

def main():
    print("=" * 80)
    print("  BUILDING PRODUCTION PACKAGE: REF4-FAST-HIERARCHICAL-EB-111A  ")
    print("=" * 80)
    ...
    # 1. Copy all src modules
    for py_file in (ROOT / "src").glob("*.py"):
        shutil.copy2(py_file, out_src / py_file.name)
    if SRC_109.exists():
        for py_file in SRC_109.glob("*.py"):
            shutil.copy2(py_file, out_src / py_file.name)
            
    # 2. Copy 109C models
    for asset in MODEL_109.glob("*"):
        if asset.is_file():
            shutil.copy2(asset, out_model / asset.name)
            
    # 3. Build True Hierarchical Empirical Bayes Lookup Table from train data
    print("Generating exact Beta-Binomial Hierarchical EB Tables from train data...")
    train = pd.read_csv(DATA_PATH, low_memory=False)
    sys.path.insert(0, str(ROOT / "scripts"))
    from build_eb_lookup import build_hierarchical_eb_tables
    eb_tables = build_hierarchical_eb_tables(train, prior_weight_global=100.0, prior_weight_hand=50.0)
    
    with open(out_model / "eb_pitcher_hand_lookup.json", "w", encoding="utf-8") as f:
        json.dump(eb_tables, f, indent=2)
    print(f"  • Saved eb_pitcher_hand_lookup.json ({len(eb_tables['pitcher_hand_lookup'])} pairs)")
    
    # 4. Write manifest.json
    manifest_109 = json.loads((MODEL_109 / "manifest.json").read_text(encoding="utf-8"))
    manifest_109["pipeline"] = "REF4-FAST-HIERARCHICAL-EB-111A"
    manifest_109["description"] = "109C Backbone + True Beta-Binomial Empirical Bayes + Fast Residuals"
    manifest_109["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest_109["eb_global_prior"] = eb_tables["global_prior"]
    manifest_109["eb_reliability_k"] = 25.0
    manifest_109["r_seeds_selected"] = [42, 1, 2]  # 3 CB + 3 LGB + 2 XGB for high speed
    manifest_109["f_seeds_selected"] = [42, 1, 2]
    manifest_109["v54_seeds"] = [42, 1, 2, 3, 4, 10]
    
    with open(out_model / "manifest.json", "w", encoding="utf-8") as f:
        json.dump(manifest_109, f, indent=2)
    ...
    script_code = '''# Production inference entry point for REF4-FAST-HIERARCHICAL-EB-111A
import json
import os
import sys
import time
...
def main():
    ...
    eb_tables = json.loads((MODEL / "eb_pitcher_hand_lookup.json").read_text(encoding="utf-8"))
    
    ph_map = eb_tables["pitcher_hand_lookup"]
    p_map = eb_tables["pitcher_baseline"]
    glob_p = float(eb_tables["global_prior"])
    ...
    if regular.any():
        ...
        # 111A: True Hierarchical Empirical Bayes Hand Matchup Table Lookup (Vectorized)
        pid_series = test_reg["pitcher_id"].astype(str) if "pitcher_id" in test_reg.columns else pd.Series([""] * len(test_reg))
        bhand_series = test_reg["batter_hand"].astype(str) if "batter_hand" in test_reg.columns else pd.Series([""] * len(test_reg))
        pair_keys = pid_series + "_" + bhand_series
        
        p_eb = pair_keys.map(ph_map).fillna(pid_series.map(p_map)).fillna(glob_p).to_numpy(float)
        
        k_prior = float(meta.get("eb_reliability_k", 25.0))
        n_p_reg = n_p_raw[regular]
        p_rate_reg = p_rate_raw[regular]
        
        rel_weight = n_p_reg / (n_p_reg + k_prior)
        p_shrunk = rel_weight * p_rate_reg + (1.0 - rel_weight) * p_eb
    ...
    if regular.any() and w_refined_call > 0.0 and (MODEL / "refined_call_expert.cbm").exists():
        ...
        p_call = safe_cb_predict_proba(cb_call, x3_cb)[:, 1]
        p[regular] = p[regular] + w_refined_call * (p_call - 0.50)

    high_pocket_mask = regular & (n_p_raw > 120) & (p_rate_raw > 0.65)
    ...
    # 111A: Streamlined High-Speed Orthogonal Residual Ensemble (3 CB + 3 LGB + 2 XGB)
    w_super_resid = float(meta.get("w_super_resid", 0.085))
    r_seeds = meta.get("r_seeds_selected", [42, 1, 2])
    if regular.any() and w_super_resid > 0.0:
        ...
        # Categorical typing for LGBM
        X_lgb_full = X_reg_full.copy()
        for c in X_lgb_full.select_dtypes(include=["object", "category"]).columns:
            X_lgb_full[c] = X_lgb_full[c].astype("category")
        ...
        for s in r_seeds:
            ...
            if lgb_path.exists():
                ...
                resid_reg_preds.append(safe_lgb_predict(lgb_m, X_lgb_full))
                
            if s in [42, 1]:  # 2 XGB models
    ...
    f_seeds = meta.get("f_seeds_selected", [42, 1, 2])
