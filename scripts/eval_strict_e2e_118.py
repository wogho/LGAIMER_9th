...
Evaluates 113A Champion Base (1121.90 LB) vs 118 Heavy 5-Fold OOF Command Model.
"""
import json
import os
import shutil
...
def evaluate_production_package(pkg_path: Path, val_df: pd.DataFrame, n_rows: int = 15000):
    ...
    sandbox = Path(tempfile.mkdtemp(prefix="strict_118_"))
    try:
        pkg_dir = sandbox / "pkg"
...
def main():
    print("=" * 80)
    print("  STRICT REAL END-TO-END EVALUATION: 113A (1121.90 LB) vs 118 HEAVY OOF  ")
    print("=" * 80)
    ...
    candidates = [
        ...
        ("118 Heavy 5-Fold OOF Command", ROOT / "model/REF4-HEAVY-OOF-COMMAND-118/production_package"),
    ]
    ...
    print("\n" + "=" * 80)
    print("  FINAL 118 SCOREBOARD (TRUE END-TO-END HOLD-OUT EVALUATION)  ")
    print("=" * 80)
    print(f"{'Candidate':<35} | {'Brier Score':<12} | {'BSS (%)':<10} | {'Delta vs 113A':<14} | {'Mean P':<8} | {'Speed'}")
    print("-" * 80)
    base_bss = results[0]["bss"]
    for r in results:
        delta = r["bss"] - base_bss
        print(f"{r['name']:<35} | {r['brier']:<12.6f} | {r['bss']:<10.4f} | {delta:<+14.4f} | {r['p_mean']:<8.4f} | {r['time']:.1f}s")
    print("=" * 80)
