#!/usr/bin/env python3
"""Strict 8-Gate Pre-Submission Verification Suite for 077A."""
...
ZIP_PATH = ROOT / "output/submit_ref4_super_champion_077.zip"
...
def main():
    print("=" * 75)
    print("      ZERO-RISK 8-GATE PRE-SUBMISSION VERIFICATION: 077A         ")
    print("=" * 75)
    ...
    # Extract to sandbox
    temp_dir = tempfile.mkdtemp(prefix="verify_077_")
    ...
    try:
        ...
        # ----------------------------------------------------
        # Gate 4: Official Dummy Test
        # ----------------------------------------------------
        data_dir = sandbox / "data"
        out_dir = sandbox / "output"
        data_dir.mkdir(parents=True, exist_ok=True)
        
        # Load sample real rows for dummy columns
        raw = pd.read_csv(TRAIN_PATH, nrows=50, low_memory=False)
        ...
        # ----------------------------------------------------
        val_reg = raw.loc[raw.game_type != "F"].iloc[:1200]
        val_fut = raw.loc[raw.game_type == "F"].iloc[:300]
        ...
        # ----------------------------------------------------
        # Pick 8 diverse rows (4 regular, 4 futures)
