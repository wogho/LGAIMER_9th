#!/usr/bin/env python3
"""Build Production Package 089A: Champion 087A Backbone (1104.61 LB) + Explicit Form-Gap (batter_pitcher_form_gap) Residual Scaler + Pocket Downshift Post-Processor."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-089A'
...
SCRIPT_CONTENT_089 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-089A."""
...
def main():
    ...
    # 087A Champion: 20 Pure L2 Multi-Seed Deep Hierarchical Per-Season Active State Engine (75 features)
    ...
    # NEW 089A: Explicit batter_pitcher_form_gap Residual Scaler on Regular Games
    form_gap_scale = float(meta.get("form_gap_scale", 0.015))
    if regular.any() and form_gap_scale > 0.0:
        p_prev1 = test["asof_pitcher_prev1_game_success_rate"].fillna(0.523766).to_numpy() if "asof_pitcher_prev1_game_success_rate" in test.columns else np.full(len(test), 0.523766)
        b_prev1 = test["asof_batter_prev1_game_success_rate"].fillna(0.523766).to_numpy() if "asof_batter_prev1_game_success_rate" in test.columns else np.full(len(test), 0.523766)
        form_gap = (p_prev1 - b_prev1).astype(np.float32)
        p = np.where(regular, p + form_gap_scale * form_gap, p)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 087A Models & Building 089A Package ===")
    
    # Copy all 087A model assets intact
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 089A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_087_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-089A'
    manifest['gate_scale'] = 0.08
    manifest['r_expert_lgbm_weight'] = 0.05
    manifest['w_deep_hierarchical'] = 0.08
    manifest['form_gap_scale'] = 0.015
    manifest['pocket_downshift_val'] = -0.012
    manifest['v54_seeds'] = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    manifest['notes'] = "087A Champion Backbone (1104.61 LB) + Explicit Form-Gap (batter_pitcher_form_gap) Residual Scaler (scale=0.015) + Pocket Downshift Post-Processor (-0.012)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 089A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_089.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_089, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_089.zip'
    if zip_path.exists():
        zip_path.unlink()
