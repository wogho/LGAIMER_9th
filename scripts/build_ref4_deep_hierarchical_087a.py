#!/usr/bin/env python3
"""Build Production Package 087A with 100% Aligned Per-Season Active State Reconstruction 75-Feature Pure L2 Residual Engine."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-087A'
...
SCRIPT_CONTENT_087 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-087A."""
...
def main():
    ...
    # NEW 087A: 20 Pure L2 Multi-Seed Deep Hierarchical Per-Season Active State Engine (75 features)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Extracting Per-Season Career Priors & Training 20 Pure L2 75-Feature Model ===")
    ...
    for s in v54_seeds:
        ...
        print(f"  • Seed {s}: Deep Pure L2 CatBoost & LightGBM trained on 75 Per-Season Active State Reconstructed features.")
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-087A'
    ...
    manifest['notes'] = "082A Champion Backbone (1104.12 LB) + Per-Season Active State Reconstruction 75-Feature Pure L2 Residual Engine (w=0.08) strictly on R"
    ...
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_087.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_087, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_087.zip'
    if zip_path.exists():
        zip_path.unlink()
