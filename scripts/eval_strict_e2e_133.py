...
Evaluates 113A Champion Base (1121.90 LB) vs 133 Orthogonal Residual Network (Convex Blend).
"""
import json
import os
import shutil
...
def evaluate_production_package(pkg_path: Path, val_df: pd.DataFrame, n_rows: int = 15000):
    ...
    sandbox = Path(tempfile.mkdtemp(prefix="strict_133_"))
    try:
        pkg_dir = sandbox / "pkg"
...
def main():
    print("=" * 80)
    print("  STRICT REAL END-TO-END EVALUATION: 113A (1121.90 LB) vs 133 ORTHO RESIDUAL  ")
    print("=" * 80)
    ...
    candidates = [
        ...
        ("133 Orthogonal Residual Network", ROOT / "model/REF4-ORTHO-RESID-133/production_package"),
    ]
    ...
    print("\n" + "=" * 80)
    print("  FINAL 133 SCOREBOARD (TRUE END-TO-END HOLD-OUT EVALUATION)  ")
    print("=" * 80)
