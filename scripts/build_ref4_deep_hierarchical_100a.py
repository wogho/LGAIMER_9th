#!/usr/bin/env python3
"""Build Production Package 100A: Milestone Champion Backbone (40 Pure L2 Models, Depth=7, Leaves=45, 20 Seeds) + Contrastive Axes Shape Rescaling (Axes 4, 5, 6)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-100A'
...
SOURCE_099_MODEL = ROOT / 'model/REF4-DEEP-HIERARCHICAL-099A/production_package/model'
SOURCE_099_SRC = ROOT / 'model/REF4-DEEP-HIERARCHICAL-099A/production_package/src'
SOURCE_099_REQS = ROOT / 'model/REF4-DEEP-HIERARCHICAL-099A/production_package/requirements.txt'
...
SCRIPT_CONTENT_100 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-100A."""
...
def main():
    ...
    # Pre-build v54 features for 100A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
    ...
    # 100A MILESTONE BACKBONE: 40 Pure L2 Models (20 Seeds CatBoost + 20 Seeds LightGBM) Retrained on Hierarchical Reliability Residuals
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Retraining 40 Pure L2 Models (20 Seeds) on 099A Champion Baseline (Depth=7, Leaves=45, Iters=350) ===")
    ...
    for s in v54_seeds:
        ...
        print(f"  • Seed {s}: Trained High-Capacity Deep L2 CatBoost & LightGBM (Total {len(tr_res_preds)} models)")
    ...
    print("\n=== Step 2: Copying Champion 099A Assets & Manifest Setup ===")
    for f in SOURCE_099_MODEL.iterdir():
    ...
    for f in SOURCE_099_SRC.iterdir():
        if f.is_file():
    ...
    shutil.copy2(SOURCE_099_REQS, PROD_DIR / 'requirements.txt')
    
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_099_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-100A'
    ...
    manifest['notes'] = "100A Milestone Champion Package: 40 High-Capacity Pure L2 Models (20 Seeds, Depth=7, Leaves=45) on 099A Baseline"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print(f"Updated manifest.json for 100A (deep_mean_offset = {deep_mean_offset:.6f})")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_100.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_100, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_100.zip'
    if zip_path.exists():
        zip_path.unlink()
