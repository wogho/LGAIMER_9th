...
EXTRAS=[f'x_{r}_{c}' for r in ('cur_succ','cur_mid') for c in ('adv','onb','sh','bs')]+[f'h1_{r}_{c}' for r in ('cur_ball','cur_rev','cur_str') for c in ('sh','bs')];COLS=BASE+STATE_COLUMNS+EXTRAS+['target_hist_pitcher_count_n','target_hist_pitcher_count_delta']
def prep(x):
...
def main():
 ...
 walk=extras(add_state_walkforward(raw.drop(columns=['row_id','control_success']),2025));st=extras(add_state_for_cutoff(dummy.drop(columns=['row_id','control_success']),raw.drop(columns=['row_id','control_success'])))
 ...
 m=cb.CatBoostClassifier(iterations=300,learning_rate=.05,depth=6,l2_leaf_reg=5,loss_function='Logloss',thread_count=-1,random_seed=42,allow_writing_files=False,verbose=False);m.fit(cb.Pool(xtrain,label=y,cat_features=cat,feature_names=COLS),verbose=False);out=ROOT/'model/COMBO-FULL-002';out.mkdir(parents=True,exist_ok=True);m.save_model(out/'model.cbm');(out/'feature_columns.json').write_text(json.dumps(COLS,ensure_ascii=False,indent=2));lookup.to_csv(out/'pitcher_count_lookup.csv',index=False);prior_bundle(raw,out);pred=m.predict_proba(cb.Pool(xtest,cat_features=cat,feature_names=COLS))[:,1];pd.DataFrame({'row_id':test.row_id,'control_success':pred}).to_csv(out/'test_predictions.csv',index=False);report={'experiment_id':'COMBO-FULL-002','official_train_only':True,'test_used_for_training':False,'external_data_used':False,'train_rows':len(raw),'test_rows':len(test),'feature_count':len(COLS),'tree_count':m.tree_count_,'lookup_rows':len(lookup),'prediction_min':float(pred.min()),'prediction_max':float(pred.max()),'status':'PASS_FULL_TRAIN_MODEL','submission_status':'HOLD'};(out/'full_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2));print(json.dumps(report,ensure_ascii=False,indent=2))
