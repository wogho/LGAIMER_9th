...
def main():
 ...
 at,av,lookup,_=build_pitcher_count_state_target_history(raw.loc[tr].copy(),raw.loc[va].copy(),smoothing=100.0);state=pd.concat([state.reset_index(drop=True),pd.concat([at,av],ignore_index=True).reset_index(drop=True)],axis=1);cols=BASE+STATE_COLUMNS+EXTRAS+['target_hist_pitcher_count_n','target_hist_pitcher_count_delta'];x,ci=prep(state[cols]);tp=cb.Pool(x.loc[tr],label=y[tr],cat_features=ci,feature_names=cols);vp=cb.Pool(x.loc[va],label=y[va],cat_features=ci,feature_names=cols);results={}
 for seed in (7,21):
  m=cb.CatBoostClassifier(iterations=300,learning_rate=.05,depth=6,l2_leaf_reg=5,loss_function='Logloss',eval_metric='Logloss',thread_count=-1,random_seed=seed,allow_writing_files=False,verbose=False,early_stopping_rounds=50);m.fit(tp,eval_set=vp,verbose=False);p=m.predict_proba(vp)[:,1];results[str(seed)]={'feature_count':len(cols),'best_iteration':int(m.get_best_iteration()),'brier':br(y[va],p),'mean':float(p.mean())}
 report={'experiment_id':'ASOF-COUNT-COMBO-SEEDS-001','environment':{'python':platform.python_version(),'catboost':cb.__version__},'official_train_only':True,'test_used':False,'external_data_used':False,'valid_year':2024,'lookup_rows':len(lookup),'results':results,'brier_range':max(v['brier'] for v in results.values())-min(v['brier'] for v in results.values()),'status':'HOLD'};out=ROOT/'model/ASOF-COUNT-COMBO-SEEDS-001';out.mkdir(parents=True,exist_ok=True);(out/'screen_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2));print(json.dumps(report,ensure_ascii=False,indent=2))
