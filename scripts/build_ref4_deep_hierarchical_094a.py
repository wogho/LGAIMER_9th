#!/usr/bin/env python3
"""Build Production Package 094A: 091A Champion Backbone (1104.6158 LB) + Regime-Separated Pitcher Form-Gap Synergy Engine (Futures-Specific Form-Gap Residual Amplification)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-094A'
...
SCRIPT_CONTENT_094 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-094A."""
...
def main():
    ...
    # 094A NEW: Regime-Separated Pitcher Form-Gap Synergy Engine (Futures-Specific Form Gap Amplification)
    # Pitcher prev1 game success rate - asof pitcher success rate
    if futures.any():
        prev1_p = test["asof_pitcher_prev1_game_success_rate"].fillna(0.523766).to_numpy() if "asof_pitcher_prev1_game_success_rate" in test.columns else np.full(len(test), 0.523766)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 091A Models & Building 094A Package ===")
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 094A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_091_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-094A'
    ...
    manifest['futures_form_gap_scale'] = 0.025
    manifest['v54_seeds'] = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    manifest['notes'] = "091A Champion Backbone (1104.6158 LB) + Regime-Separated Pitcher Form-Gap Synergy Engine on Futures (f_form_scale=0.025)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 094A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_094.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_094, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_094.zip'
    if zip_path.exists():
        zip_path.unlink()
