#!/usr/bin/env python3
"""Run REF4-TRAINONLY-R-BASE-EXPERT-059A forward OOF training and evaluation."""
...
def main():
    ...
    for valid_year in (2023, 2024):
        ...
        print(f"Fitting R-only CatBoost Regressor on {mask_r.sum()} R-rows...", flush=True)
        p = c['r_expert_params']
        ...
        model.fit(X_train_v3.loc[mask_r], res_train[mask_r], sample_weight=decay_weights[mask_r], cat_features=CAT_V2)
        model_path = OUT / f"r_expert_{valid_year}.cbm"
        model.save_model(model_path)
        
        # Predict on valid
        r_res_valid = model.predict(X_valid_v3)
        r_pred_valid = np.clip(base_valid + r_res_valid + 0.0052, 1e-6, 1.0 - 1e-6)
        ...
        fold_df = pd.DataFrame({
            ...
            'r_expert_prediction': r_pred_valid,
    ...
    result = {
        ...
        'candidate_name': f'R_base_expert_catboost_blend_w{primary_w:.2f}_plus_fixed_051_and_F_psych',
    ...
    paths = [
        ...
        OUT / 'r_expert_2023.cbm', OUT / 'r_expert_2024.cbm',
        ...
        ROOT / '01_제약과금지사항.md',
        ROOT / 'scripts/preflight_ref4_r_base_expert_059a.py',
        ROOT / 'scripts/run_ref4_r_base_expert_059a.py',
        ROOT / 'scripts/verify_ref4_r_base_expert_059a.py'
    ...
    print(f"\nExecution finished in {time.time()-t0:.2f}s. Result: {result['candidate_status']}, Promotion: {promotion}")
