#!/usr/bin/env python3
"""Strict Row-Independence Audit & Sandbox Verification for submit_ref4_macro_leap_075."""
...
AUDIT_DIR = ROOT / 'model/REF4-MACRO-LEAP-CHAMPION-075A/audit_sandbox'
...
def main():
    ...
    with zipfile.ZipFile(ZIP_PATH, 'r') as zf:
        ...
        print(f"ZIP Check: 'requirements.txt' in zip: {'requirements.txt' in namelist}")
        print(f"ZIP Check: 'script.py' in zip: {'script.py' in namelist}")
    ...
    print(f"1. Running Baseline inference on {n_probe} probe rows (1군 & 2군 mixed)...")
    ...
    print(f"4. Running Single-Row isolated inference on 8 diverse test rows (4 Regular, 4 Futures)...")
    reg_indices = list(np.where(test_probe.game_type == 'R')[0][:4])
    fut_indices = list(np.where(test_probe.game_type == 'F')[0][:4])
    selected_indices = reg_indices + fut_indices
    isolated_diffs = []
    for idx in selected_indices:
        ...
        gt = test_probe.game_type.iloc[idx]
        print(f"   Row {idx:4d} ({gt}): Base={p_base[idx]:.8f}, Single={p_single:.8f}, Diff={diff_single:.3e}")
