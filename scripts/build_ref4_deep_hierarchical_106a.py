...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-106A'
...
SCRIPT_CONTENT_106 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-106A."""
...
def main():
    ...
    # Pre-build v54 features for 106A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 103A Models & Base Assets ===")
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-106A'
    manifest['gate_scale'] = 0.08
    manifest['r_expert_lgbm_weight'] = 0.05
    manifest['w_deep_hierarchical'] = 0.12
    manifest['k_reliability_shrinkage'] = 5.0
    ...
    print("\n=== Step 2: Writing script.py and Packaging submit_ref4_deep_hierarchical_106.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_106, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_106.zip'
    if zip_path.exists():
        zip_path.unlink()
