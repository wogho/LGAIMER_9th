...
BUNDLE = ROOT / "model/COMBO-FULL-002"
...
def main():
    t = pd.read_csv(ROOT / "data/test.csv", encoding="utf-8-sig")
    out = t.copy()
    pp = pd.read_csv(BUNDLE / "asof_pitcher_id_prior.csv")
    bp = pd.read_csv(BUNDLE / "asof_batter_id_prior.csv")
    mp = pd.read_csv(BUNDLE / "asof_pitchmix_prior.csv")
    out = out.merge(pp, on="pitcher_id", how="left", validate="many_to_one").merge(bp, on="batter_id", how="left", validate="many_to_one").merge(mp, on="pitcher_id", how="left", validate="many_to_one")
    curp = (out.asof_pitcher_n.fillna(0) - out.prior_n_x.fillna(0)).clip(lower=0)
    curb = (out.asof_batter_n.fillna(0) - out.prior_n_y.fillna(0)).clip(lower=0)
    ...
    for rc, label, prior, cur, n in specs:
        out[label] = ((n.fillna(0) * out[rc].fillna(0) - out[prior].fillna(0)) / cur).where(cur > 0)
    ...
    full = (out.balls_before == 3) & (out.strikes_before == 2); pa = ((out.balls_before == 0) & out.strikes_before.isin([1,2])) | ((out.balls_before == 1) & (out.strikes_before == 2)); ba = ((out.balls_before == 2) & (out.strikes_before == 0)) | ((out.balls_before == 3) & out.strikes_before.isin([0,1]))
    out["count_state"] = np.select([full, pa, ba], ["full_count", "ahead_pitcher", "ahead_batter"], default="neutral")
    for f in EXTRAS:
        z = f.split("_"); out[f] = out["_".join(z[1:3])] * out[z[3]]
    lk = pd.read_csv(BUNDLE / "pitcher_count_lookup.csv")
    out = out.merge(lk, on=["pitcher_id", "count_state"], how="left", validate="many_to_one")
    out[["target_hist_pitcher_count_n","target_hist_pitcher_count_delta"]] = out[["target_hist_pitcher_count_n","target_hist_pitcher_count_delta"]].fillna(0)
    ...
    pred = model.predict_proba(cb.Pool(x, cat_features=cats, feature_names=COLS))[:,1]
    result = pd.DataFrame({"row_id": t.row_id, "control_success": pred}); (BUNDLE / "isolated_submission.csv").write_text(result.to_csv(index=False)); (ROOT / "output").mkdir(exist_ok=True); result.to_csv(ROOT / "output/submission.csv", index=False)
    assert len(x.columns) == 73 and np.isfinite(pred).all() and ((pred >= 0) & (pred <= 1)).all()
    print(json.dumps({"rows": len(result), "features": len(x.columns), "categorical": cats, "min": float(pred.min()), "max": float(pred.max()), "status": "PASS_ISOLATED_INFERENCE", "submission_status": "HOLD"}, ensure_ascii=False))
