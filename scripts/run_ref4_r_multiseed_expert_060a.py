#!/usr/bin/env python3
"""Run REF4-TRAINONLY-R-MULTISEED-EXPERT-060A forward OOF training and evaluation."""
...
OUT = ROOT / 'model/REF4-TRAINONLY-R-MULTISEED-EXPERT-060A'
...
def main():
    ...
    for valid_year in (2023, 2024):
        ...
        for s_idx, seed_val in enumerate(seeds):
            print(f"Fitting Seed {s_idx+1}/{len(seeds)} (seed={seed_val}) on {mask_r.sum()} R-rows...", flush=True)
            ...
            model_path = OUT / f"r_expert_{valid_year}_seed_{seed_val}.cbm"
        ...
        fold_df = pd.DataFrame({
            ...
            'r_expert_multiseed_prediction': r_pred_valid,
    ...
    result = {
        ...
        'candidate_name': f'R_multiseed_expert_3seeds_blend_w{primary_w:.2f}_plus_fixed_051_and_F_psych',
    ...
    paths = [
        ...
        ROOT / '01_제약과금지사항.md',
        ROOT / 'scripts/preflight_ref4_r_multiseed_expert_060a.py',
        ROOT / 'scripts/run_ref4_r_multiseed_expert_060a.py',
        ROOT / 'scripts/verify_ref4_r_multiseed_expert_060a.py'
    ] + [ROOT / p for p in c['base_oof'].values()]
    
    for yr in (2023, 2024):
        for s_val in seeds:
            paths.append(OUT / f"r_expert_{yr}_seed_{s_val}.cbm")
    ...
    print(f"\n060A Execution finished in {time.time()-t0:.2f}s. Result: {result['candidate_status']}, Promotion: {promotion}")
