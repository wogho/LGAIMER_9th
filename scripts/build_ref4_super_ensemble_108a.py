#!/usr/bin/env python3
"""Build Production Package 108A: 107A Champion Backbone + 10-Model (5 CatBoost + 5 LightGBM) Dual-Engine Orthogonal Residual Booster (w=0.07)."""
...
EXP_DIR = ROOT / 'model/REF4-SUPER-ENSEMBLE-108A'
...
SCRIPT_CONTENT_108 = '''"""Offline inference entry point for REF4-SUPER-ENSEMBLE-108A."""
...
def main():
    ...
    # 108A NEW: Dual-Engine (5 CatBoost + 5 LightGBM) Orthogonal Residual Booster Ensemble (w=0.07)
    w_super_resid = float(meta.get("w_super_resid", 0.07))
    ...
    if regular.any() and w_super_resid > 0.0:
        ...
        for s in r_seeds:
            ...
            if cb_path.exists():
                ...
                resid_preds.append(cb_m.predict(X_61))
                
            lgb_path = MODEL / f"super_resid_lgb_seed{s}.txt"
            if lgb_path.exists():
                lgb_m = lgb.Booster(model_file=str(lgb_path))
                resid_preds.append(lgb_m.predict(X_61))
                
        if resid_preds:
            p_res_mean = np.mean(resid_preds, axis=0)
            p = np.where(regular, p + w_super_resid * p_res_mean, p)
...
def build_package():
    ...
    print("\n=== Step 2: Training 10 Dual-Engine Residual Boosters on Full Train (1,475,092 rows) ===")
    ...
    r_seeds = [42, 1, 2, 3, 4]
    print(f"\nFitting 10 Dual-Engine Boosters (5 CatBoost + 5 LightGBM) on {len(X_61):,} Regular rows...")
    for s in r_seeds:
        # CatBoost
        ...
        dtrain = lgb.Dataset(X_61, label=y_resid_reg)
        bst = lgb.train(lgb_params, dtrain, num_boost_round=160)
        bst.save_model(str(MODEL_DIR / f"super_resid_lgb_seed{s}.txt"))
        print(f"  • Seed {s}: CatBoost & LightGBM fitted and saved")
        
    print("\n=== Step 3: Updating Manifest for 108A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_107_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-SUPER-ENSEMBLE-108A'
    manifest['w_super_resid'] = 0.07
    manifest['r_seeds'] = r_seeds
    manifest['notes'] = "108A Champion 107A (1115.2561 LB) + 10-Model (5 CatBoost + 5 LightGBM) Dual-Engine Orthogonal Residual Booster (w=0.07)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 108A")
    
    print("\n=== Step 4: Packaging submit_ref4_super_ensemble_108.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_108, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_super_ensemble_108.zip'
    if zip_path.exists():
        zip_path.unlink()
