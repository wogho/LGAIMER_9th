#!/usr/bin/env python3
"""Build Production Package 090A: 087A Champion Backbone (1104.61 LB) + Game-Type Regulated Futures Calibration (F-Regime Scale Optimization + Separate Global Calibration)."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-090A'
...
SCRIPT_CONTENT_090 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-090A."""
...
def main():
    ...
    # 090A: Regulated Futures (F) regime scale adjustment
    if futures.any():
    ...
    # 090A: Game-Type Regulated Calibration Shift (R: +0.0052, F: +0.0018)
    r_shift = float(meta.get("global_shift", 0.0052))
    f_shift = float(meta.get("futures_shift", 0.0018))
    p = np.where(regular, p + r_shift, p + f_shift)
    ...
    # Brier-Optimized Pocket Downshift Post-Processor (Strictly on Regular Games)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 087A Models & Building 090A Package ===")
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 090A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_087_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-090A'
    manifest['gate_scale'] = 0.08
    manifest['r_expert_lgbm_weight'] = 0.05
    manifest['w_deep_hierarchical'] = 0.08
    manifest['global_shift'] = 0.0052
    manifest['futures_shift'] = 0.0018
    manifest['pocket_downshift_val'] = -0.012
    manifest['v54_seeds'] = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    manifest['notes'] = "087A Champion Backbone (1104.61 LB) + Game-Type Regulated Futures Calibration (R: +0.0052, F: +0.0018) + Strict Regime Isolation"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 090A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_090.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_090, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_090.zip'
    if zip_path.exists():
        zip_path.unlink()
