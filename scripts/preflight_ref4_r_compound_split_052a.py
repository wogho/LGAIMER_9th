...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'model/REF4-TRAINONLY-R-COMPOUND-SPLIT-052A'
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def main():
 ...
 for prefix in ('f_psych','comparison'):
  a=json.loads((ROOT/c[prefix+'_attestation']).read_text());ck(prefix+'_audit',a.get('overall_status')=='AUDIT_VERIFIED' and a.get('mismatch_count')==0 and a.get('oof_predictions_sha256')==sha(ROOT/c[prefix+'_predictions']),{'status':a.get('overall_status'),'performance':a.get('performance_status'),'mismatch':a.get('mismatch_count'),'hash':a.get('oof_predictions_sha256')==sha(ROOT/c[prefix+'_predictions'])})
 ck('fixed_contract',c['candidate_count']==1 and c['parameter_sweep'] is False and len(c['compound_specs'])==6 and c['single_context_feature_count']==40 and c['compound_context_feature_count']==24 and c['total_feature_count']==64 and c['ridge_alpha']==10000.0,{'candidate':c['candidate_count'],'sweep':c['parameter_sweep'],'specs':len(c['compound_specs']),'features':c['total_feature_count'],'alpha':c['ridge_alpha']});ck('scope',not c['production_fit'] and not c['test_read'] and not c['zip_creation'],{'production':c['production_fit'],'test':c['test_read'],'zip':c['zip_creation']});ck('rollback',sha(ROOT/c['preserve_zip'])=='1f62100e3901410df69390549dbf7d7c80ecb9ae83dc829e0763254bff84bf6a',sha(ROOT/c['preserve_zip']))
