#!/usr/bin/env python3
"""
REF4-CHAMPION-RESTORE-112A Builder.
Candidate 112A: Full 1126.45 Champion Architecture with Real Trained Transition Gate.
"""
import json
import os
import shutil
...
MODEL_DIR = ROOT / "model/REF4-CHAMPION-RESTORE-112A/production_package/model"
SRC_DIR = ROOT / "model/REF4-CHAMPION-RESTORE-112A/production_package/src"
OUT_DIR = ROOT / "model/REF4-CHAMPION-RESTORE-112A/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_112A.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 112A: REF4-CHAMPION-RESTORE-112A  ")
    print("=" * 80)
    
    # Write production script.py
    script_code = '''# Production inference entry point for REF4-CHAMPION-RESTORE-112A
import json
import os
import sys
import time
...
def main():
    ...
    # Real League Transition Gate
    if (MODEL / "transition_gate.cbm").exists():
        tg_m = CatBoostRegressor()
        tg_m.load_model(str(MODEL / "transition_gate.cbm"))
        prev1_p_all = test["asof_pitcher_prev1_game_success_rate"].fillna(0.523766).to_numpy(float) if "asof_pitcher_prev1_game_success_rate" in test.columns else p_rate_raw
        p_anchor_all = np.clip(0.70 * p_rate_raw + 0.30 * prev1_p_all, 0.05, 0.95)
        b_all = test["balls_before"].fillna(0).to_numpy(float) if "balls_before" in test.columns else np.zeros(len(test))
        s_all = test["strikes_before"].fillna(0).to_numpy(float) if "strikes_before" in test.columns else np.zeros(len(test))
        li_all = test["li"].fillna(0.98).to_numpy(float) if "li" in test.columns else np.full(len(test), 0.98)
        
        tg_X = pd.DataFrame({
            "game_type": test["game_type"].astype(str),
            "pitcher_hand": test["pitcher_hand"].astype(str),
            "batter_hand": test["batter_hand"].astype(str),
            "log_pitcher_n": np.log1p(n_p_raw),
            "asof_pitcher_success_rate": p_rate_raw,
            "asof_pitcher_prev1_game_success_rate": prev1_p_all,
            "momentum_drift": prev1_p_all - p_rate_raw,
            "li": li_all,
            "count_pressure": (b_all - s_all) / 3.0,
            "p_anchor": p_anchor_all
        })
        tg_res = safe_cb_predict(tg_m, tg_X)
        p = p + 0.05 * tg_res
    ...
    # 112A: 109C Tri-Bridge 15-Model Ensemble
