#!/usr/bin/env python3
"""Build Production Package 101A: New Champion 100A Backbone (1105.6845 LB) + Upgraded LightGBM R-Expert (Rounds=450, Leaves=45)."""
...
sys.path.insert(0, str(ROOT / 'model/REF4-DEEP-HIERARCHICAL-100A/production_package'))

EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-101A'
...
SOURCE_100_MODEL = ROOT / 'model/REF4-DEEP-HIERARCHICAL-100A/production_package/model'
SOURCE_100_SRC = ROOT / 'model/REF4-DEEP-HIERARCHICAL-100A/production_package/src'
SOURCE_100_REQS = ROOT / 'model/REF4-DEEP-HIERARCHICAL-100A/production_package/requirements.txt'
...
SCRIPT_CONTENT_101 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-101A."""
...
def main():
    ...
    # Pre-build v54 features for 101A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
    ...
    # Regular 1군 split profile + Upgraded LightGBM R-Expert
    if regular.any():
        ...
        # 101A UPGRADED: LightGBM R-Expert (Rounds=450, Leaves=45)
        lgbm_model = lgb.Booster(model_file=str(MODEL / 'r_expert_lgbm_upgraded.txt'))
        x3_lgb = x3.copy()
        for col in CAT_V2:
    ...
    # 100A CHAMPION: 40 Pure L2 Models (20 Seeds CatBoost + 20 Seeds LightGBM) Retrained on Hierarchical Reliability Residuals
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Training Upgraded LightGBM R-Expert (Rounds=450, Leaves=45) ===")
    ...
    ps = pd.read_pickle(SOURCE_100_MODEL / "pitcher_snapshots.pkl")
    bs = pd.read_pickle(SOURCE_100_MODEL / "batter_snapshots.pkl")
    ms = pd.read_pickle(SOURCE_100_MODEL / "pitchmix_snapshots.pkl")
    tm = str(SOURCE_100_MODEL / "trackman_prior_features.csv")
    
    x3_tr, base3_tr = build_v3_features(train_reg, 0.523766, ps, bs, ms, tm)
    y_tr = train_reg.control_success.to_numpy(float)
    resid_r = y_tr - base3_tr - 0.0052
    
    x3_lgb_tr = x3_tr.copy()
    for col in CAT_V2:
        if col in x3_lgb_tr.columns:
            x3_lgb_tr[col] = x3_lgb_tr[col].astype('category')
            
    lgb_tr = lgb.Dataset(x3_lgb_tr, label=resid_r)
    params = {
    ...
    lgb_r_expert = lgb.train(params, lgb_tr, num_boost_round=450)
    lgb_r_expert.save_model(str(MODEL_DIR / 'r_expert_lgbm_upgraded.txt'))
    print("  • Successfully trained Upgraded LightGBM R-Expert (Rounds=450, Leaves=45)")

    print("\n=== Step 2: Copying Champion 100A 40 Pure L2 Models & Assets ===")
    for f in SOURCE_100_MODEL.iterdir():
        if f.is_file() and f.name != 'manifest.json':
            shutil.copy2(f, MODEL_DIR / f.name)
            
    for f in SOURCE_100_SRC.iterdir():
        if f.is_file():
            shutil.copy2(f, SRC_DIR / f.name)
            
    shutil.copy2(SOURCE_100_REQS, PROD_DIR / 'requirements.txt')
    
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_100_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-101A'
    ...
    manifest['notes'] = "101A Champion 100A Backbone (1105.6845 LB) + Upgraded LightGBM R-Expert (Rounds 450, Leaves 45)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 101A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_101.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_101, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_101.zip'
    if zip_path.exists():
        zip_path.unlink()
