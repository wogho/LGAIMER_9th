...
EXP_DIR = ROOT / 'model/REF4-SUPER-ENSEMBLE-108B'
...
SCRIPT_CONTENT_108B = '''"""Offline inference entry point for REF4-SUPER-ENSEMBLE-108B."""
...
def main():
    ...
    if regular.any() and w_super_resid > 0.0:
        ...
        phys_df = extract_physics_features(test)
        X_physics = pd.concat([X_61, phys_df], axis=1)
...
def build_package():
    ...
    print("\n=== Step 2: Training 10 Physics-Augmented Residual Boosters on Full Train (1,475,092 rows) ===")
    ...
    phys_reg = extract_physics_features(raw_reg)
    X_physics = pd.concat([X_61, phys_reg], axis=1)
    
    r_seeds = [42, 1, 2, 3, 4]
    print(f"\nFitting 10 Physics-Augmented Boosters (5 CatBoost + 5 LightGBM) on {len(X_physics):,} Regular rows...")
    for s in r_seeds:
        ...
        dtrain = lgb.Dataset(X_physics, label=y_resid_reg)
    ...
    print("\n=== Step 3: Updating Manifest for 108B ===")
    ...
    print("\n=== Step 4: Packaging submit_ref4_super_ensemble_108B.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_108B, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_super_ensemble_108B.zip'
    if zip_path.exists():
        zip_path.unlink()
