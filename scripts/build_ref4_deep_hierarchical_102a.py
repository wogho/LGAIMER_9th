...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-102A'
...
SCRIPT_CONTENT_102 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-102A."""
...
def main():
    ...
    # Pre-build v54 features for 102A Hierarchical Baseline L2 residual engine
    v54_feat = None
    p_hb_hier = None
    if regular.any():
    ...
    # 102A: 40 Pure L2 Models (20 Seeds CatBoost + 20 Seeds LightGBM) Retrained with Leverage Index Features
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Retraining 40 Pure L2 Models with Leverage Index Features (40 Features Total) ===")
    ...
    lev_df_tr = build_leverage_features(train_reg)
    v54_train_feat = pd.concat([v54_train_feat, lev_df_tr], axis=1)
    ...
    for s in v54_seeds:
        ...
        print(f"  • Seed {s}: Trained High-Capacity Deep L2 CatBoost & LightGBM with Leverage Index (Total {len(tr_res_preds)} models)")
    ...
    print("\n=== Step 2: Copying Champion 100A Base Assets & Manifest Setup ===")
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-102A'
    ...
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_102.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_102, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_102.zip'
    if zip_path.exists():
        zip_path.unlink()
