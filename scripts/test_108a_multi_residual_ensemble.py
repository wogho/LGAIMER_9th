#!/usr/bin/env python3
"""Evaluate 108A: Multi-Model (CatBoost + LightGBM + XGBoost) Orthogonal Residual Booster Ensemble on 107A Champion."""
...
def main():
    print("=== Evaluating Multi-Model (CatBoost + LightGBM + XGBoost) Residual Booster Ensemble ===")
    t0 = time.time()
    ...
    # Train 103A on 2019-2023 train set
    ...
    # Train 3 CatBoost + 3 LightGBM + 3 XGBoost residual boosters
    r_seeds = [42, 1, 2]
    preds_val_models = []
    
    print("\n--- Training Multi-Model Residual Boosters ---")
    for s in r_seeds:
        ...
        p_cb = cb.predict(X_val_61)
        preds_val_models.append(p_cb)
        ...
        p_lgb = lgb_m.predict(X_val_61)
        preds_val_models.append(p_lgb)
        ...
        p_xgb = xgb_m.predict(X_val_61)
        preds_val_models.append(p_xgb)
        print(f"  • Seed {s}: CatBoost, LightGBM, XGBoost trained successfully")
        
    p_multi_resid_val = np.mean(preds_val_models, axis=0)
    
    print("\n--- Testing Multi-Model Residual Booster Weights ---")
    for w in [0.03, 0.05, 0.06, 0.07, 0.08, 0.10, 0.12]:
        p_cand = p_103_val.copy()
        p_cand[reg_val] = p_cand[reg_val] + w * p_multi_resid_val
        p_cand = np.clip(p_cand, 1e-5, 1 - 1e-5)
        bss_c = brier_skill_score(y_val, p_cand)
        delta = bss_c - bss_base
        print(f"  Multi-Model Ensemble Scale {w:.2f} -> BSS: {bss_c:.4f} (Δ {delta:+.4f} pt)")
