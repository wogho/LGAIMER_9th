#!/usr/bin/env python3
"""
REF4-TK-PITCHMIX-DRIFT-115B Builder.
Candidate 115B: 115A (114B + Bradley-Terry) + 9th Repo Trackman Pitchmix Count Deviations.
"""
import json
import os
import shutil
...
MODEL_115A = ROOT / "model/REF4-BT-SEASON-DECOMP-115A/production_package/model"
SRC_115A = ROOT / "model/REF4-BT-SEASON-DECOMP-115A/production_package/src"
FROZEN_9TH = ROOT / "model/frozen_9th_tables"
OUT_DIR = ROOT / "model/REF4-TK-PITCHMIX-DRIFT-115B/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_115B.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 115B: REF4-TK-PITCHMIX-DRIFT-115B  ")
    print("=" * 80)
    ...
    if SRC_115A.exists():
        for py_file in SRC_115A.glob("*.py"):
            shutil.copy2(py_file, out_src / py_file.name)
            
    # 2. Copy all 115A models
    for asset in MODEL_115A.glob("*"):
    ...
    manifest = json.loads((MODEL_115A / "manifest.json").read_text(encoding="utf-8"))
    manifest["pipeline"] = "REF4-TK-PITCHMIX-DRIFT-115B"
    manifest["description"] = "115A + 9th Repo Trackman Count Mixture Deviations"
    ...
    script_code = '''# Production inference entry point for REF4-TK-PITCHMIX-DRIFT-115B
import json
import os
import sys
import time
