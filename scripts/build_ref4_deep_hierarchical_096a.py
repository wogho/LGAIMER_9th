...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-096A'
...
SCRIPT_CONTENT_096 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-096A."""
...
def main():
    ...
    if regular.any() and w_hier_scale > 0.0:
        overall_prior = float(meta.get("prior", 0.523766))
        rel_weight = n_p_raw / (n_p_raw + k_prior)
        shrunk_rate = overall_prior + rel_weight * (p_rate_raw - overall_prior)
        rate_diff = shrunk_rate - p_rate_raw
        p = np.where(regular, p + w_hier_scale * rate_diff, p)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 091A Models & Building 096A Package ===")
    ...
    print("\n=== Step 2: Updating Manifest and Packaging 096A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_091_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-096A'
    ...
    manifest['w_hier_scale'] = 0.035
    manifest['k_prior'] = 25.0
    manifest['v54_seeds'] = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    manifest['notes'] = "091A Champion Backbone (1104.6158 LB) + Hierarchical Reliability Shrinkage Engine (1146pt Mechanism, k=25.0, w_hier=0.035)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 096A")
    
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_096.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_096, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_096.zip'
    if zip_path.exists():
        zip_path.unlink()
