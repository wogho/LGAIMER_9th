#!/usr/bin/env python3
"""
REF4-ORTHO-RESID-133 Production Packager & Builder.
Integrates 113A Champion (1121.9040 LB) + 6-Model Orthogonal Residual Corrector (Convex Blend).
Strictly NO zombie components, strictly Convex Blend (w=0.030).
"""
import json
import os
import shutil
...
ORTHO_133_MODEL = ROOT / "model/REF4-ORTHO-RESID-133/production_package/model"
OUT_DIR = ROOT / "model/REF4-ORTHO-RESID-133/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_133.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 133: REF4-ORTHO-RESID-133  ")
    print("=" * 80)
    ...
    # 3. Copy ortho residual models
    for file in ORTHO_133_MODEL.glob("*"):
        if file.is_file() and not (out_model / file.name).exists():
            shutil.copy2(file, out_model / file.name)
        
    # 4. Write manifest.json
    manifest = json.loads((MODEL_113A / "manifest.json").read_text(encoding="utf-8"))
    manifest["pipeline"] = "REF4-ORTHO-RESID-133"
    manifest["description"] = "113A Champion (1121.9040 LB) + Orthogonal Residual Corrector Convex Blend"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["w_ortho_resid_convex"] = 0.030
    ...
    script_code = '''# Production inference entry point for REF4-ORTHO-RESID-133
import json
import os
import sys
import time
...
def predict_ortho_resid_133(df: pd.DataFrame, model_dir: Path, priors: dict) -> np.ndarray:
    ...
    X_full = pd.concat([feat_base, feat_ortho], axis=1)
    
    seeds = [13301, 13302, 13303]
    preds = []
    for s in seeds:
        cb_m = CatBoostRegressor()
        cb_m.load_model(str(model_dir / f"ortho_resid_cb_seed{s}.cbm"))
        preds.append(safe_cb_predict(cb_m, X_full))
        
        lgb_m = lgb.Booster(model_file=str(model_dir / f"ortho_resid_lgb_seed{s}.txt"))
        preds.append(safe_lgb_predict(lgb_m, X_full))
        
    r_mean = np.mean(preds, axis=0)
    alpha = 0.035
    p_anchor = np.clip(0.70 * p_rate + 0.30 * prev1_p, 0.05, 0.95)
    p_ortho_model = np.clip(p_anchor + alpha * np.tanh(r_mean / (alpha + 1e-5)), 0.02, 0.98)
    return p_ortho_model
...
def main():
    ...
    # =========================================================================
    # 133: STRICT CONVEX BLEND with Orthogonal Residual Corrector
    # p_133 = (1 - w) * p_113A + w * p_ortho_resid
    # =========================================================================
    w_ortho = float(meta.get("w_ortho_resid_convex", 0.030))
    p_ortho = predict_ortho_resid_133(test, MODEL, priors)
    
    p = (1.0 - w_ortho) * p + w_ortho * p_ortho
