#!/usr/bin/env python3
"""Build Production Package 107A: 103A Champion Backbone (1107.1970 LB) + Hierarchical Reliability Engine (1146.3952 LB) Dedicated 61-Feature Orthogonal Residual Booster (w=0.05)."""
...
EXP_DIR = ROOT / 'model/REF4-SUPER-ENSEMBLE-107A'
...
SCRIPT_CONTENT_107 = '''"""Offline inference entry point for REF4-SUPER-ENSEMBLE-107A."""
...
def main():
    ...
    # Safe universal extraction of pitch level stats
    ...
    # 103A CHAMPION: 40 Pure L2 Models (20 CatBoost + 20 LightGBM)
    ...
    # 103A Refined Call 6-Class Multi-Class Sub-Expert (w=0.04)
    ...
    # 107A NEW: Hierarchical Reliability Engine (1146.3952 LB) Dedicated 61-Feature Orthogonal Residual Booster (w=0.05)
    w_super_resid = float(meta.get("w_super_resid", 0.05))
    if regular.any() and w_super_resid > 0.0 and (MODEL / "super_resid_booster.cbm").exists():
        r_booster = CatBoostRegressor()
        r_booster.load_model(str(MODEL / "super_resid_booster.cbm"))
        
        X_61, _ = build_v5_deep_61_features(test, profile_path=MODEL / "team_asof_profile.json", prior=0.523766)
        prev1_p_val = test["asof_pitcher_prev1_game_success_rate"].to_numpy(float) if "asof_pitcher_prev1_game_success_rate" in test.columns else p_rate_raw
        prev1_p_val = np.where(np.isnan(prev1_p_val), p_rate_raw, prev1_p_val)
        b_rate_val = test["asof_batter_success_rate"].fillna(0.523766).to_numpy(float) if "asof_batter_success_rate" in test.columns else np.full(len(test), 0.523766)
        
        X_61["form_gap"] = prev1_p_val - b_rate_val
        X_61["anchor_p"] = p
        
        resid_preds = r_booster.predict(X_61)
        p = np.where(regular, p + w_super_resid * resid_preds, p)
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Copying Champion 103A Base Models & Assets ===")
    ...
    print("\n=== Step 2: Training Hierarchical Reliability Engine Dedicated 61-Feature Residual Booster on Full Train (1,475,092 rows) ===")
    ...
    regime = json.loads((SOURCE_103_MODEL / "f_regime_meta.json").read_text())
    meta_103 = json.loads((SOURCE_103_MODEL / "manifest.json").read_text(encoding='utf-8'))
    
    # Save team as of profile
    ...
    p_103_full = predict_103a_full(raw, SOURCE_103_MODEL, meta_103, priors, ps, bs, ms, tm, seeds, regime)
    y_full = raw["control_success"].to_numpy(float)
    anchor_resid = y_full - p_103_full
    
    # Extract 61 features for Regular rows
    reg_mask = raw["game_type"] != "F"
    raw_reg = raw[reg_mask].copy().reset_index(drop=True)
    y_resid_reg = anchor_resid[reg_mask]
    
    X_61, _ = build_v5_deep_61_features(raw_reg, profile_path=MODEL_DIR / "team_asof_profile.json", prior=0.523766)
    ...
    X_61["form_gap"] = prev1_p - b_rate
    X_61["anchor_p"] = p_103_full[reg_mask]
    
    print(f"Fitting dedicated CatBoost Residual Booster on {len(X_61):,} Regular rows...")
    resid_booster = CatBoostRegressor(
        iterations=200,
        depth=5,
        learning_rate=0.04,
        l2_leaf_reg=15.0,
    ...
    resid_booster.fit(X_61, y_resid_reg)
    resid_booster.save_model(str(MODEL_DIR / "super_resid_booster.cbm"))
    print("  • Saved super_resid_booster.cbm")
    
    print("\n=== Step 3: Updating Manifest for 107A ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_103_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-SUPER-ENSEMBLE-107A'
    manifest['w_super_resid'] = 0.05
    manifest['notes'] = "107A Champion 103A Backbone (1107.1970 LB) + Hierarchical Reliability Engine (1146.3952 LB) Dedicated 61-Feature Orthogonal Residual Booster (w=0.05)"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 107A")
    
    print("\n=== Step 4: Packaging submit_ref4_super_ensemble_107.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_107, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_super_ensemble_107.zip'
    if zip_path.exists():
        zip_path.unlink()
