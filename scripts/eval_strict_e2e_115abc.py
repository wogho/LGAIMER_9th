...
Evaluates 113A Champion (1121.90 LB) vs 114B vs 115A vs 115B vs 115C.
"""
import json
import os
import shutil
...
def evaluate_production_package(pkg_path: Path, val_df: pd.DataFrame, n_rows: int = 15000):
    ...
    sandbox = Path(tempfile.mkdtemp(prefix="strict_115_"))
    try:
        pkg_dir = sandbox / "pkg"
...
def main():
    print("=" * 80)
    print("  STRICT REAL END-TO-END EVALUATION: 113A (1121.90 LB) vs 114B vs 115A vs 115B vs 115C  ")
    print("=" * 80)
    ...
    candidates = [
        ...
        ("114B Dynamic Sigmoidal", ROOT / "model/REF4-NESTED-DYNAMIC-114B/production_package"),
        ("115A BT + Season Decomp", ROOT / "model/REF4-BT-SEASON-DECOMP-115A/production_package"),
        ("115B TK Pitchmix Drift", ROOT / "model/REF4-TK-PITCHMIX-DRIFT-115B/production_package"),
        ("115C Tri-System Grand", ROOT / "model/REF4-TRI-SYSTEM-GRAND-115C/production_package"),
    ]
    ...
    print("\n" + "=" * 80)
    print("  FINAL 115 SCOREBOARD (TRUE END-TO-END HOLD-OUT EVALUATION)  ")
    print("=" * 80)
    print(f"{'Candidate':<28} | {'Brier Score':<12} | {'BSS (%)':<10} | {'Delta vs 113A':<14} | {'Speed'}")
    print("-" * 80)
    base_bss = results[0]["bss"]
    for r in results:
        delta = r["bss"] - base_bss
        print(f"{r['name']:<28} | {r['brier']:<12.6f} | {r['bss']:<10.4f} | {delta:<+14.4f} | {r['time']:.1f}s")
    print("=" * 80)
