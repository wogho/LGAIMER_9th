#!/usr/bin/env python3
"""Build Production Package 108C: 107A Champion Backbone + Full-Spectrum (1군 3D물리/카운트전략 10-Model + 2군 퓨처스 4-Model) Dual-Decoupled Residual Ensemble."""
...
EXP_DIR = ROOT / 'model/REF4-SUPER-ENSEMBLE-108C'
...
SCRIPT_CONTENT_108C = '''"""Offline inference entry point for REF4-SUPER-ENSEMBLE-108C."""
...
def main():
    ...
    # 108C NEW: Full-Spectrum Dual-Decoupled Residual Boosters
    # 1군 Regular (w=0.08)
    ...
    if regular.any() and w_super_resid > 0.0:
        ...
        phys_reg = extract_advanced_physics(test[regular].reset_index(drop=True))
    ...
    if futures.any() and w_fut_resid > 0.0:
        ...
        phys_fut = extract_advanced_physics(test[futures].reset_index(drop=True))
...
def build_package():
    ...
    print("\n=== Step 2: Training Full-Spectrum Residual Boosters on Full Train (1,475,092 rows) ===")
    ...
    phys_reg = extract_advanced_physics(raw_reg)
    X_physics_reg = pd.concat([X_61_reg, phys_reg], axis=1)
    
    r_seeds = [42, 1, 2, 3, 4]
    print(f"\nFitting 1군 10 Full-Spectrum Boosters (5 CatBoost + 5 LightGBM) on {len(X_physics_reg):,} Regular rows...")
    for s in r_seeds:
    ...
    phys_fut = extract_advanced_physics(raw_fut)
    ...
    print("\n=== Step 3: Updating Manifest for 108C ===")
    manifest_path = MODEL_DIR / 'manifest.json'
    manifest = json.loads((SOURCE_107_MODEL / 'manifest.json').read_text(encoding='utf-8'))
    manifest['version'] = 'REF4-SUPER-ENSEMBLE-108C'
    manifest['w_super_resid'] = 0.08
    manifest['w_fut_resid'] = 0.04
    manifest['r_seeds'] = r_seeds
    manifest['f_seeds'] = f_seeds
    manifest['notes'] = "108C Champion 107A + Full-Spectrum (1군 3D물리/카운트전략 10-Model w=0.08 + 2군 퓨처스 전용 4-Model w=0.04) Dual-Decoupled Residual Ensemble"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print("Updated manifest.json for 108C")
    
    print("\n=== Step 4: Packaging submit_ref4_super_ensemble_108C.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_108C, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_super_ensemble_108C.zip'
    if zip_path.exists():
        zip_path.unlink()
