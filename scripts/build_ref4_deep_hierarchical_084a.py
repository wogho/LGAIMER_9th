#!/usr/bin/env python3
"""Build Production Package 084A with 20 Pure L2 Multi-Seed (10 CatBoost + 10 LightGBM) Deep Hierarchical 65-Feature Residual Ensemble."""
...
EXP_DIR = ROOT / 'model/REF4-DEEP-HIERARCHICAL-084A'
...
from v51_deep_65_features import build_v51_deep_65_features
...
SCRIPT_CONTENT_084 = '''"""Offline inference entry point for REF4-DEEP-HIERARCHICAL-084A."""
...
from src.v51_deep_65_features import build_v51_deep_65_features
...
def main():
    ...
    # NEW 084A: 20 Pure L2 Multi-Seed Deep Hierarchical Residual Model (984pt class) strictly for Regular season (game_type == 'R')
    w_deep = float(meta.get("w_deep_hierarchical", 0.08))
    v51_seeds = meta.get("v51_seeds", [42, 1, 2, 3, 4, 10, 20, 30, 40, 50])
    if regular.any() and w_deep > 0.0:
        v51_feat, p_hb = build_v51_deep_65_features(test, profile_path=MODEL / "team_asof_profile.json", prior=float(meta.get("prior", 0.523766)))
        
        res_preds = []
        for s in v51_seeds:
            cb_m = CatBoostRegressor()
            cb_m.load_model(str(MODEL / f"deep_cb_l2_seed{s}.cbm"))
            res_preds.append(cb_m.predict(v51_feat))
            
            lgb_m = lgb.Booster(model_file=str(MODEL / f"deep_lgb_l2_seed{s}.txt"))
            res_preds.append(lgb_m.predict(v51_feat))
...
def build_package():
    t0 = time.time()
    print("=== Step 1: Training 20 Pure L2 Multi-Seed Deep Hierarchical Residual Model ===")
    ...
    v51_train_feat, p_hb_tr = build_v51_deep_65_features(train_reg, profile_path=profile_path)
    
    y_tr = train_reg.control_success.to_numpy(float)
    resid_hb_tr = (y_tr - p_hb_tr).astype(np.float32)
    
    v51_seeds = [42, 1, 2, 3, 4, 10, 20, 30, 40, 50]
    tr_res_preds = []
    ...
    for s in v51_seeds:
        ...
        cb_m.fit(v51_train_feat, resid_hb_tr)
        cb_path = MODEL_DIR / f"deep_cb_l2_seed{s}.cbm"
        cb_m.save_model(str(cb_path))
        tr_res_preds.append(cb_m.predict(v51_train_feat))
        
        # LightGBM L2
        lgb_tr = lgb.Dataset(v51_train_feat, label=resid_hb_tr)
        params = {
        ...
        tr_res_preds.append(lgb_m.predict(v51_train_feat))
        
        print(f"  • Seed {s}: Deep Pure L2 CatBoost & LightGBM trained on (y - p_hb) residual.")
    ...
    shutil.copy2(ROOT / 'src/v51_deep_65_features.py', SRC_DIR / 'v51_deep_65_features.py')
    ...
    manifest['version'] = 'REF4-DEEP-HIERARCHICAL-084A'
    ...
    manifest['v51_seeds'] = v51_seeds
    manifest['notes'] = "082A Champion Backbone (1104.12 LB) + 20 Pure L2 Multi-Seed Deep Residual Engine (w=0.08) strictly on R"
    ...
    print("\n=== Step 3: Writing script.py and Packaging submit_ref4_deep_hierarchical_084.zip ===")
    (PROD_DIR / 'script.py').write_text(SCRIPT_CONTENT_084, encoding='utf-8')
    
    zip_path = ROOT / 'output/submit_ref4_deep_hierarchical_084.zip'
    if zip_path.exists():
        zip_path.unlink()
