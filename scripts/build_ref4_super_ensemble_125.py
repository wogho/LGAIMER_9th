#!/usr/bin/env python3
"""
REF4-REGIME-13-HUB-125 Production Packager & Builder.
Integrates 113A Champion (1121.9040 LB) + Team-13 Hub Regime & ABS Boundary Inversion (Convex Blend).
Strictly NO zombie components, strictly Convex Blend (w=0.045).
"""
import json
import os
import shutil
...
REGIME_125_MODEL = ROOT / "model/REF4-REGIME-13-HUB-125/production_package/model"
OUT_DIR = ROOT / "model/REF4-REGIME-13-HUB-125/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_125.zip"

def main():
    print("=" * 80)
    print("  PACKAGING CANDIDATE 125: REF4-REGIME-13-HUB-125  ")
    print("=" * 80)
    ...
    # 3. Copy regime 13 models
    for file in REGIME_125_MODEL.glob("*"):
    ...
    manifest["pipeline"] = "REF4-REGIME-13-HUB-125"
    manifest["description"] = "113A Champion (1121.9040 LB) + Team-13 Hub Regime & ABS Boundary Inversion Convex Blend"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["w_team13_regime_convex"] = 0.045
    ...
    script_code = '''# Production inference entry point for REF4-REGIME-13-HUB-125
import json
import os
import sys
import time
...
def predict_team13_regime_125(df: pd.DataFrame, model_dir: Path, priors: dict) -> np.ndarray:
    ...
    X_full = pd.concat([feat_base, feat_regime], axis=1)
    
    seeds = [12501, 12502, 12503]
    preds = []
    for s in seeds:
        cb_m = CatBoostRegressor()
        cb_m.load_model(str(model_dir / f"team13_regime_cb_seed{s}.cbm"))
        preds.append(safe_cb_predict(cb_m, X_full))
        
        lgb_m = lgb.Booster(model_file=str(model_dir / f"team13_regime_lgb_seed{s}.txt"))
        preds.append(safe_lgb_predict(lgb_m, X_full))
        
    p_regime_model = np.mean(preds, axis=0)
    return p_regime_model
...
def main():
    ...
    # =========================================================================
    # 125: STRICT CONVEX BLEND with Team-13 Hub Regime Model
    # p_125 = (1 - w) * p_113A + w * p_team13_regime
    # =========================================================================
    w_regime = float(meta.get("w_team13_regime_convex", 0.045))
    p_regime = predict_team13_regime_125(test, MODEL, priors)
    
    p = (1.0 - w_regime) * p + w_regime * p_regime
