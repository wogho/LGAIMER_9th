#!/usr/bin/env python3
"""
REF4-DIRECT-BRIER-SIMPLEX-112C Builder.
Candidate 112C: Direct Brier Loss SLSQP Non-Negative Simplex Global Optimization + Context Gating.
"""
import json
import os
import shutil
...
OUT_DIR = ROOT / "model/REF4-DIRECT-BRIER-SIMPLEX-112C/production_package"
ZIP_OUT = ROOT / "output/submit_ref4_super_ensemble_112C.zip"

def main():
    print("=" * 80)
    print("  BUILDING CANDIDATE 112C: REF4-DIRECT-BRIER-SIMPLEX-112C  ")
    print("=" * 80)
    ...
    with open(out_model / "f_regime_meta.json", "w", encoding="utf-8") as f:
        json.dump(f_regime_1126, f, indent=2)
        
    # 4. Write manifest.json
    manifest = json.loads((MODEL_109 / "manifest.json").read_text(encoding="utf-8"))
    manifest["pipeline"] = "REF4-DIRECT-BRIER-SIMPLEX-112C"
    manifest["description"] = "Direct Brier Loss SLSQP Non-Negative Simplex Global Optimization + Context Gating"
    manifest["created_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    manifest["r_seeds"] = [42, 1, 2, 3, 4]
    manifest["f_seeds"] = [42, 1, 2, 3]
    manifest["v54_seeds"] = [42, 1, 2, 3, 4, 10]
    manifest["f_regime"] = f_regime_1126
    ...
    script_code = '''# Production inference entry point for REF4-DIRECT-BRIER-SIMPLEX-112C
import json
import os
import sys
import time
...
def main():
    ...
    # Corrected pocket shifts: small-sample mean regression
