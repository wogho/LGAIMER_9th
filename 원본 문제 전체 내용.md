...
참가자는 제출 전 아래 사항을 포함한 [전체 대회 규칙]을 반드시 확인해 주시기 바랍니다.

허용 범위를 벗어난 사전학습모델 및 가중치 사용 금지
원격 서버 기반 외부 API 사용 금지
제공된 공식 데이터 외 외부 데이터 사용 금지
평가 데이터(test.csv)의 각 행을 독립적인 예측 대상으로 처리
특히 평가 데이터의 다른 행이나 전체 평가 데이터의 분포를 이용해 특정 행의 예측값을 보정하거나 생성하는 방식은 정상적인 추론 절차로 인정되지 않습니다.

대회 규칙을 준수하지 않은 제출이 확인되는 경우, 규정에 따라 실격에 해당 할 수 있습니다.

공정한 대회 운영을 위해 참가자 여러분께서는 제출 전 대회 규칙을 충분히 확인해 주시기 바랍니다.
